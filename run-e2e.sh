...
docker compose -f docker-compose.yml -f docker-compose.jwks-test.yml --profile jwks-test up --detach jwks-stub
./copalibre init
./copalibre doctor
./copalibre start
./copalibre create-admin --organization-alias integrity-league --organization-name "Integrity League" --email admin@example.test
yarn playwright test --config playwright.compose.config.ts
