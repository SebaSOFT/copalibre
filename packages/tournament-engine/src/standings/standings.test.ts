...
const outcome = (
  ...
  scores: [
    { entrantId: a, score: aScore },
    { entrantId: b, score: bScore },
  ],
});
...
describe('computeAccounting', () => {
  ...
  it('reports zeroes for an entrant with no recorded matches', () => {
    const [row] = computeAccounting(['solo'], []);
    expect(row).toMatchObject({ played: 0, points: 0, scoreDifference: 0 });
  });

  it('ignores outcomes referencing unknown entrants', () => {
    const rows = computeAccounting(['a'], [outcome('m1', 'x', 1, 'y', 0)]);
  ...
  it('ignores a bye outcome, which is not a played match', () => {
    const rows = computeAccounting(
      ['a', 'b'],
      [{ matchId: 'm1', winnerEntrantId: 'a', scores: [{ entrantId: 'a', score: 0 }] }],
    );
    expect(rows.every((row) => row.played === 0)).toBe(true);
  });

  it('honours custom points rules', () => {
    const rows = computeAccounting(['a', 'b'], [outcome('m1', 'a', 1, 'b', 0)], {
      win: 2,
      draw: 1,
      loss: -1,
    });
...
describe('toEntrantValues', () => {
  it('exposes every accounting parameter the pipeline can reference', () => {
    const values = toEntrantValues(computeAccounting(['a', 'b'], [outcome('m1', 'a', 2, 'b', 1)]));
...
describe('computeStandings', () => {
  ...
  it('marks entrants the pipeline could not separate as sharing a rank', () => {
    const standings = computeStandings(['a', 'b'], [outcome('m1', 'a', 1, 'b', 1)], {
      ...pipeline,
  ...
  it('produces a stable golden trace for a full round-robin table', () => {
    ...
    const standings = computeStandings(ids, outcomes, pipeline);
