...
/**
 * Deterministic pseudo-random integer generated by SHA-256 hashing
 * tournament ID, stage ID, and entrant ID.
 */
export function deterministicTiebreakHash(
  ...
  const hash = createHash('sha256').update(`${tournamentId}:${stageId}:${entrantId}`).digest('hex');
