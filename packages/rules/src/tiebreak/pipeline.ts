...
/**
 * Deterministic pseudo-random 32-bit integer generated by FNV-1a hashing
 * tournament ID, stage ID, and entrant ID. Pure JS, isomorphic across Node and browser.
 */
export function deterministicTiebreakHash(
  tournamentId: string,
  stageId: string,
  entrantId: string,
): number {
  const str = `${tournamentId}:${stageId}:${entrantId}`;
  let hash = 2166136261;
...
export interface TiebreakParameterDefinition {
  ...
  readonly source: 'calculated' | 'operator-entered' | 'match-derived' | 'random' | 'manual';
}
...
export function resolveTiebreak(
  ...
  for (const [index, parameter] of pipeline.parameters.entries()) {
    ...
    for (const group of groups) {
      ...
      const split = splitByParameter(group, parameter, effectiveValues, observed, options);
    ...
    const labelText = parameterLabelText(parameter.label);
    trace.push({
      kind: 'comparator',
      id: parameter.id,
      label: `Rule ${index + 1} (${labelText})`,
      outcome: resolvedAll
        ? 'resolved'
      ...
      detail: parameter.unboundCapability
        ...
        : resolvedAll
          ? `${labelText} resolved the tie`
          : `Tie not fully resolved by ${labelText}; proceed to next comparator`,
...
function splitByParameter(
  ...
  const scored = group.map((entrantId) => {
    let raw: unknown;
    if (parameter.id === 'random' || parameter.source === 'random') {
      const tournamentId = options?.seedContext?.tournamentId ?? 'default-tournament';
      const stageId = options?.seedContext?.stageId ?? 'default-stage';
      raw = deterministicTiebreakHash(tournamentId, stageId, entrantId);
    } else if (
      (parameter.id === 'manual' ||
        parameter.source === 'manual' ||
        parameter.source === 'operator-entered') &&
      options?.manualTiebreakerPoints &&
      entrantId in options.manualTiebreakerPoints
    ) {
      raw = options.manualTiebreakerPoints[entrantId];
    } else if (parameter.ratio) {
      raw = ratioValue(values[entrantId] ?? {}, parameter.ratio);
    } else {
      raw = values[entrantId]?.[parameter.id];
    }
