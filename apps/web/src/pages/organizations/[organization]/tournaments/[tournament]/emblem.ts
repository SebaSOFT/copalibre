...
export const GET: APIRoute = async ({ params }) => {
  ...
  const url = `${getApiBaseUrl()}/organizations/${encodeURIComponent(organization)}/tournaments/${encodeURIComponent(tournament)}/emblem`;
  try {
    ...
    return new Response('Failed to fetch tournament emblem', { status: 502 });
  }
};
