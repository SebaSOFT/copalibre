/**
 * Portuguese translations, keyed to match `public-messages.en.ts`'s IDs
 * exactly. Best-effort translation; native-speaker review is a later
 * pass (owner's explicit choice).
 */
export const messages: Record<string, string> = {
  'publicWeb.bracket.highlightEntrant': 'Destacar percurso de {entrant}',
  'publicWeb.bracket.highlightHint':
    'Selecione um participante para seguir seu percurso. Selecione novamente ou pressione Escape para limpar. As partidas futuras mostram o possível percurso de vitórias.',
  'publicWeb.layout.skipToContent': 'Pular para o conteúdo',
  'publicWeb.layout.navAriaLabel': 'Principal',
  'publicWeb.layout.footer': 'Publicado com CopaLibre — AGPL-3.0',

  'publicWeb.resultState.live': 'AO VIVO',
  'publicWeb.resultState.upcoming': 'PROGRAMADO',
  'publicWeb.resultState.final': 'FINAL',
  'publicWeb.resultState.disputed': 'EM DISPUTA',
  'publicWeb.resultState.winner': 'VENCEU',
  'publicWeb.resultState.loser': 'PERDEU',
  'publicWeb.resultState.tbd': 'A DEFINIR',
  'publicWeb.resultState.cancelled': 'CANCELADO',

  'publicWeb.resultReason.administrativeLoss': 'DERROTA ADM.',
  'publicWeb.resultReason.walkover': 'W/O',
  'publicWeb.resultReason.forfeitAbandonment': 'ABANDONO',
  'publicWeb.resultReason.disqualified': 'DESCLASSIFICADO',
  'publicWeb.resultReason.didNotFinish': 'NÃO TERMINOU',

  'publicWeb.legend.heading': 'Legenda',

  'publicWeb.ruleset.heading': 'Regulamento',
  'publicWeb.ruleset.valueYes': 'Sim',
  'publicWeb.ruleset.valueNo': 'Não',
  'publicWeb.ruleset.format.singleElimination': 'Eliminação simples',
  'publicWeb.ruleset.format.doubleElimination': 'Eliminação dupla',
  'publicWeb.ruleset.format.roundRobin': 'Round-robin',
  'publicWeb.ruleset.format.roundRobinSingleLeg': 'Round-robin (jogo único)',
  'publicWeb.ruleset.format.roundRobinHomeAway': 'Round-robin (ida e volta)',
  'publicWeb.ruleset.format.league': 'Liga',
  'publicWeb.ruleset.format.swiss': 'Sistema suíço',
  'publicWeb.ruleset.format.gauntlet': 'Gauntlet',
  'publicWeb.ruleset.format.bracketGroups': 'Grupos com chave',
  'publicWeb.ruleset.format.customBracket': 'Chave personalizada',
  'publicWeb.ruleset.format.ffaBracket': 'Chave FFA',
  'publicWeb.ruleset.format.ffaLeague': 'Liga FFA',

  'publicWeb.standings.heading': 'Classificação',
  'publicWeb.standings.tableLayouts': 'Layouts de tabela',
  'publicWeb.standings.filterByClub': 'Filtrar por clube',
  'publicWeb.standings.closeDialog': 'Fechar diálogo',
  'publicWeb.home.pageTitle': 'CopaLibre — Operação de torneios',
  'publicWeb.home.organizationsLabel': 'Organizações',
  'publicWeb.standings.empty': 'A classificação aparece quando a primeira partida é disputada.',
  'publicWeb.standings.team': 'Equipe',
  'publicWeb.standings.played': 'PJ',
  'publicWeb.standings.points': 'Pts',
  'publicWeb.standings.grain.series': 'Esta tabela conta um resultado por série.',
  'publicWeb.standings.grain.match': 'Esta tabela conta um resultado por partida disputada.',
  'publicWeb.standings.column.series.label': 'Séries',
  'publicWeb.standings.column.series.shortLabel': 'S',
  'publicWeb.standings.playerDialog.loadingPlaceholder': 'Carregando detalhes do jogador...',
  'publicWeb.standings.playerDialog.loadingFetch': 'Carregando...',
  'publicWeb.standings.playerDialog.columnTournament': 'Torneio',
  'publicWeb.standings.playerDialog.columnTeam': 'Equipe',
  'publicWeb.standings.playerDialog.columnRole': 'Função',
  'publicWeb.standings.playerDialog.loadError': 'Não foi possível carregar os detalhes do jogador.',

  'publicWeb.hero.liveCount': '{count} AO VIVO',
  'publicWeb.hero.noLiveMatches': 'NENHUMA PARTIDA AO VIVO',
  'publicWeb.hero.tournamentEmblemAlt': 'Escudo de {name}',
  'publicWeb.hero.tournamentEmblemPlaceholderAlt': 'Nenhum escudo do torneio enviado',

  'publicWeb.bracket.roundAriaLabel': '{branch} — rodada {round}',
  'publicWeb.bracket.roundHeading': 'Rodada {round}',

  'publicWeb.broadcastStatus.note':
    'Os resultados são atualizados automaticamente quando há conexão. Caso contrário, esta página já traz tudo.',

  'publicWeb.matchGrid.heading': 'Partidas',
  'publicWeb.matchGrid.empty': 'Ainda não há partidas programadas.',

  'publicWeb.series.ariaLabel': 'Série melhor de {bestOf}: {home} a {away}',
  'publicWeb.series.gameWonHome': 'Jogo {number}: vencido pelo mandante',
  'publicWeb.series.gameWonAway': 'Jogo {number}: vencido pelo visitante',
  'publicWeb.series.gameCurrent': 'Jogo {number}: em andamento',
  'publicWeb.series.gameUpcoming': 'Jogo {number}: ainda a disputar',
  'publicWeb.series.gameNotRequired': 'Jogo {number}: não será disputado',
  'publicWeb.series.pending': 'Série indefinida em {home}–{away}',
  'publicWeb.series.decided': '{winner} venceu a série',
  'publicWeb.series.aggregate': 'No agregado {home}–{away}',

  'publicWeb.live.usingLastKnown': 'Mostrando o último estado conhecido.',

  'publicWeb.tournamentPage.description':
    'Resultados, classificação e regulamento de {tournament}.',

  'publicWeb.livePage.title': 'Ao vivo',
  'publicWeb.livePage.breadcrumb': 'Trilha de navegação',
  'publicWeb.livePage.noMatches': 'Nenhum jogo ao vivo',
  'publicWeb.livePage.nextKickoff': 'Próximo jogo agendado',
  'publicWeb.livePage.noUpcoming': 'Nenhum próximo jogo agendado.',
  'publicWeb.livePage.seriesHeading': 'Série',
  'publicWeb.livePage.upcomingHeading': 'Próximas',
  'publicWeb.livePage.leadersHeading': 'Líderes',

  'publicWeb.bracketPage.title': 'Chave',

  'publicWeb.playerProfile.heading': 'Perfil do Jogador',
  'publicWeb.playerProfile.age': 'Idade: {age}',
  'publicWeb.playerProfile.nationality': 'Nacionalidade: {country}',
  'publicWeb.playerProfile.historyHeading': 'Histórico de Competições',
  'publicWeb.playerProfile.careerStatsHeading': 'Estatísticas da Carreira',
  'publicWeb.playerProfile.noHistory': 'Nenhum histórico de competições registrado.',
  'publicWeb.playerProfile.noStats': 'Nenhuma estatística da carreira registrada.',
  'publicWeb.playerProfile.close': 'Fechar',
  'publicWeb.playerProfile.backTo': '← Voltar para {name}',
  'publicWeb.playerProfile.photoAlt': '{name}',
  'publicWeb.playerProfile.photoPlaceholderAlt': 'Nenhuma foto enviada',
  'publicWeb.playerProfile.tournamentStatsHeading': 'Estatísticas do Torneio',
  'publicWeb.playerProfile.tournamentTotalHeading': 'Total do Torneio',
  'publicWeb.playerProfile.matchByMatchHeading': 'Partida a Partida',
  'publicWeb.playerProfile.noTournamentStats':
    'Nenhuma estatística de torneio registrada para este jogador.',
  'publicWeb.playerProfile.matchColumnHeading': 'Partida',
  'publicWeb.playerProfile.matchLabel': 'Etapa {stage} · Partida {match}',

  'publicWeb.tournamentsPage.title': 'Torneios',
  'publicWeb.tournamentsPage.liveHeading': 'Ao Vivo e Ativos',
  'publicWeb.tournamentsPage.upcomingHeading': 'Próximos',
  'publicWeb.tournamentsPage.finishedHeading': 'Encerrados e Arquivo',
  'publicWeb.tournamentsPage.empty': 'Nenhum torneio publicado encontrado.',
  'publicWeb.tournamentsPage.champion': 'Campeão',
  'publicWeb.tournamentsPage.runnerUp': 'Vice-campeão',
  'publicWeb.tournamentsPage.thirdPlace': 'Terceiro lugar',
  'publicWeb.tournamentsPage.viewDetails': 'Ver torneio',
  'publicWeb.orgPage.featuredHeading': 'Destaque',
  'publicWeb.orgPage.clubsHeading': 'Clubes',
  'publicWeb.orgPage.noClubs': 'Ainda não há clubes registrados.',
  'publicWeb.orgPage.emblemAlt': 'Escudo de {name}',
  'publicWeb.orgPage.emblemPlaceholderAlt': 'Nenhum escudo enviado',
  'publicWeb.orgPage.notFoundTitle': 'Organização não encontrada',
  'publicWeb.orgPage.notFoundBody': 'Não existe nenhuma organização neste endereço.',

  'publicWeb.matchesView.pageTitle': 'Partidas',
  'publicWeb.matchesView.filterAll': 'Todas',
  'publicWeb.matchesView.filterLive': 'Ao vivo',
  'publicWeb.matchesView.filterUpcoming': 'Próximas',
  'publicWeb.matchesView.filterFinal': 'Finalizadas',
  'publicWeb.matchesView.empty': 'Ainda não há partidas neste escopo.',
  'publicWeb.matchesView.clockAriaLabel': 'Tempo decorrido: {time}',
  'publicWeb.matchesView.venueAriaLabel': 'Local: {venue}',
  'publicWeb.matchesView.scheduledAtAriaLabel': 'Início: {time}',
  'publicWeb.matchesView.latestEventAriaLabel': 'Último evento: {event}',
  'publicWeb.matchesView.zoneGroupAriaLabel': 'Zona/grupo: {scope}',
  'publicWeb.matchesView.stageFilterAriaLabel': 'Filtrar por fase',
  'publicWeb.matchesView.zoneFilterAriaLabel': 'Filtrar por zona',
  'publicWeb.matchesView.groupFilterAriaLabel': 'Filtrar por grupo',
  'publicWeb.matchesView.stateFilterAriaLabel': 'Filtrar por estado',
  'publicWeb.matchesView.positionInGroup': '{group} — posição nº {position}',
  'publicWeb.matchesView.position': 'Posição nº {position}',
  'publicWeb.matchesView.decidedBy': 'Decidido por: {factor}',
  'publicWeb.matchesView.decidedByAriaLabel':
    'O fator decisivo entre posições empatadas. Um organizador autorizado pode ver a explicação completa no painel de controle.',
  'publicWeb.matchesView.fullTraceHeading': 'Rastro completo do comparador de classificação',
  'publicWeb.matchesView.seeAll': 'Ver todas as partidas',

  'publicWeb.notFound.pageTitle': 'Página não encontrada — CopaLibre',
  'publicWeb.notFound.heading': 'Página não encontrada',
  'publicWeb.notFound.body': 'Nenhum conteúdo existe neste endereço.',
  'publicWeb.notFound.homeLink': 'Voltar ao início',

  'publicWeb.ticker.label': 'Ticker do torneio',
  'publicWeb.ticker.leader': 'Líder',
  'publicWeb.ticker.versus': 'VS',
  'publicWeb.tvStats.homeSide': 'Casa',
  'publicWeb.tvStats.awaySide': 'Fora',
  'publicWeb.tvStats.points': 'Pontos',
  'publicWeb.tvStats.pointsShort': 'Pts',
  'publicWeb.tvStats.unnamedActor': 'Competidor {reference}',
  'publicWeb.tvStats.scheduledMatches': 'Partidas agendadas',
  'publicWeb.tvStats.status': 'Estado',
  'publicWeb.tvStats.inProgress': 'Em andamento',
  'publicWeb.tvStats.matchesPlayed': 'Partidas disputadas',
  'publicWeb.tvStats.totalScored': 'Total marcado',
  'publicWeb.tvStats.averagePerMatch': 'Média por partida',
  'publicWeb.tvStats.highestResult': 'Maior resultado',
  'publicWeb.tvStats.championTitle': 'Campeão do torneio',
  'publicWeb.tvStats.tableLeaderTitle': 'Líder da tabela',
  'publicWeb.tvStats.standingsRecord': '1.º · {points} pts · {played} jogadas',
  'publicWeb.tvStats.grandFinalRecord': 'Vencedor da grande final ({winner} – {loser})',

  // 0223 — ticker conditions, bracket stage chrome, public header.
  'publicWeb.ticker.pause': 'Pausar o placar',
  'publicWeb.ticker.resume': 'Retomar o placar',
  'publicWeb.ticker.stale': 'Últimos resultados conhecidos — a ligação ao vivo foi perdida',
  'publicWeb.ticker.empty': 'Ainda não há jogos a comunicar',
  'publicWeb.bracket.stageAriaLabel': 'Chave',
  'publicWeb.bracket.zoneJumpAriaLabel': 'Ir para a chave de uma zona',
  'publicWeb.bracket.scrollAriaLabel': 'Gráfico da chave, desloca-se lateralmente',
  'publicWeb.bracket.legendLabel': 'Legenda da chave',
  'publicWeb.bracket.outcomeAdvancing': 'Avança',
  'publicWeb.bracket.outcomeEliminated': 'Eliminado',
  'publicWeb.bracket.outcomePending': 'Ainda por decidir',
  'publicWeb.bracket.outlineHeading': 'Rondas e ramos',
  'publicWeb.bracket.championshipTitle': 'Final',
  'publicWeb.bracket.contextAriaLabel': 'Esta partida na chave',
  'publicWeb.bracket.contextViewFullLink': 'Ver a chave completa',
  'publicWeb.header.openMenu': 'Abrir o menu',
  'publicWeb.header.closeMenu': 'Fechar o menu',
  'publicWeb.header.language': 'Idioma',
  'publicWeb.header.navHome': 'Início',
  'publicWeb.header.navHelp': 'Ajuda',
  'publicWeb.header.navApiReference': 'Referência da API',
  'publicWeb.header.controlPanel': 'Painel de controlo',

  // TvDashboard.tsx's own chrome (openspec 0225 task 2.6).
  'publicWeb.tvDashboard.noMatchesScheduled': 'Nenhuma partida agendada no momento',
  'publicWeb.tvDashboard.standingsUnavailable': 'Tabela de classificação indisponível',
  'publicWeb.tvDashboard.clubColumn': 'Clube',
  'publicWeb.tvDashboard.playedColumn': 'PJ',
  'publicWeb.tvDashboard.noTopPerformers': 'Nenhum destaque registrado',
  'publicWeb.tvDashboard.focalPanelLabel': 'Painel principal da transmissão',
  'publicWeb.tvDashboard.matchEventsLabel': 'Eventos da partida',
  'publicWeb.tvDashboard.statsAndTablesLabel': 'Estatísticas e tabelas do torneio',
  'publicWeb.tvDashboard.sidebarSectionsLabel': 'Seções laterais',
  'publicWeb.tvDashboard.standingsTab': 'Classificação',
  'publicWeb.tvDashboard.performersTab': 'Melhores jogadores',
  'publicWeb.tvDashboard.statisticsTab': 'Estatísticas',
  'publicWeb.tvDashboard.bracketTab': 'Chave',
  'publicWeb.tvDashboard.bracketRound': 'Rodada',
  'publicWeb.tvDashboard.bracketMatch': 'Partida',
  'publicWeb.tvDashboard.possession': 'Posse',
  'publicWeb.tvDashboard.penalty': 'Penalidade',
  'publicWeb.completion.heading': 'Progresso do torneio',
  'publicWeb.completion.summary': '{resolved} de {total} partidas disputadas',
  'publicWeb.completion.complete': 'Concluído',
  'publicWeb.completion.inProgress': 'Em andamento',
  'publicWeb.completion.unmeasured': 'Nenhuma partida agendada',

  'publicWeb.matchTimeline.heading': 'Linha do tempo da partida',
  'publicWeb.matchTimeline.empty': 'Nenhum evento disponível no momento.',
  'publicWeb.matchOfficials.heading': 'Árbitros',
  'publicWeb.matchOfficials.scheduleNotPublished': 'O horário ainda não foi publicado.',
  'publicWeb.matchOfficials.noneAssigned': 'Nenhum árbitro atribuído.',
  'publicWeb.matchRosters.heading': 'Escalações',
  'publicWeb.matchRosters.empty': 'As escalações ainda não estão disponíveis.',
  'publicWeb.matchRosters.teamEmpty': 'Escalação ainda não disponível.',
  'publicWeb.matchRosters.columnNumber': 'N.º',
  'publicWeb.matchRosters.columnPlayer': 'Jogador',
  'publicWeb.matchRosters.columnRoles': 'Funções',
  'publicWeb.matchRosters.columnOnField': 'Em campo',
  'publicWeb.matchRosters.onFieldYes': 'Sim',
  'publicWeb.matchRosters.onFieldNo': 'Não',
};
