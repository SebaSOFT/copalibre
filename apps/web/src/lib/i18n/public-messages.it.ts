/**
 * Italian translations, keyed to match `public-messages.en.ts`'s IDs exactly
 *. Best-effort translation; native-speaker review is a later pass
 * (owner's explicit choice).
 */
export const messages: Record<string, string> = {
  'publicWeb.bracket.highlightEntrant': 'Evidenzia il percorso di {entrant}',
  'publicWeb.bracket.highlightHint':
    'Seleziona un partecipante per seguirne il percorso. Selezionalo di nuovo o premi Escape per cancellare. Gli incontri futuri mostrano il possibile percorso in caso di vittoria.',
  'publicWeb.layout.skipToContent': 'Vai al contenuto',
  'publicWeb.layout.navAriaLabel': 'Principale',
  'publicWeb.layout.footer': 'Pubblicato con CopaLibre — AGPL-3.0',

  'publicWeb.resultState.live': 'IN DIRETTA',
  'publicWeb.resultState.upcoming': 'PROGRAMMATO',
  'publicWeb.resultState.final': 'FINALE',
  'publicWeb.resultState.disputed': 'IN DISPUTA',
  'publicWeb.resultState.winner': 'VINTO',
  'publicWeb.resultState.loser': 'PERSO',
  'publicWeb.resultState.tbd': 'DA DEFINIRE',
  'publicWeb.resultState.cancelled': 'ANNULLATO',

  'publicWeb.resultReason.administrativeLoss': 'SCONFITTA AMM.',
  'publicWeb.resultReason.walkover': 'W/O',
  'publicWeb.resultReason.forfeitAbandonment': 'RITIRO',
  'publicWeb.resultReason.disqualified': 'SQUALIFICATO',
  'publicWeb.resultReason.didNotFinish': 'NON COMPLETATO',

  'publicWeb.legend.heading': 'Legenda',

  'publicWeb.ruleset.heading': 'Regolamento',
  'publicWeb.ruleset.valueYes': 'Sì',
  'publicWeb.ruleset.valueNo': 'No',
  'publicWeb.ruleset.format.singleElimination': 'Eliminazione diretta',
  'publicWeb.ruleset.format.doubleElimination': 'Doppia eliminazione',
  'publicWeb.ruleset.format.roundRobin': 'Girone all’italiana',
  'publicWeb.ruleset.format.roundRobinSingleLeg': 'Girone (andata)',
  'publicWeb.ruleset.format.roundRobinHomeAway': 'Girone (andata e ritorno)',
  'publicWeb.ruleset.format.league': 'Campionato',
  'publicWeb.ruleset.format.swiss': 'Sistema svizzero',
  'publicWeb.ruleset.format.gauntlet': 'Gauntlet',
  'publicWeb.ruleset.format.bracketGroups': 'Gironi a eliminazione',
  'publicWeb.ruleset.format.customBracket': 'Tabellone personalizzato',
  'publicWeb.ruleset.format.ffaBracket': 'Tabellone FFA',
  'publicWeb.ruleset.format.ffaLeague': 'Campionato FFA',

  'publicWeb.standings.heading': 'Classifica',
  'publicWeb.standings.tableLayouts': 'Layout di tabella',
  'publicWeb.standings.filterByClub': 'Filtra per club',
  'publicWeb.standings.closeDialog': 'Chiudi finestra',
  'publicWeb.home.pageTitle': 'CopaLibre — Gestione tornei',
  'publicWeb.home.organizationsLabel': 'Organizzazioni',
  'publicWeb.standings.empty': 'La classifica appare dopo la prima partita giocata.',
  'publicWeb.standings.team': 'Squadra',
  'publicWeb.standings.played': 'PG',
  'publicWeb.standings.points': 'Pti',
  'publicWeb.standings.grain.series': 'Questa tabella conta un risultato per serie.',
  'publicWeb.standings.grain.match': 'Questa tabella conta un risultato per partita giocata.',
  'publicWeb.standings.column.series.label': 'Serie',
  'publicWeb.standings.column.series.shortLabel': 'S',
  'publicWeb.standings.playerDialog.loadingPlaceholder':
    'Caricamento dei dettagli del giocatore...',
  'publicWeb.standings.playerDialog.loadingFetch': 'Caricamento...',
  'publicWeb.standings.playerDialog.columnTournament': 'Torneo',
  'publicWeb.standings.playerDialog.columnTeam': 'Squadra',
  'publicWeb.standings.playerDialog.columnRole': 'Ruolo',
  'publicWeb.standings.playerDialog.loadError': 'Impossibile caricare i dettagli del giocatore.',

  'publicWeb.hero.liveCount': '{count} IN DIRETTA',
  'publicWeb.hero.noLiveMatches': 'NESSUNA PARTITA IN DIRETTA',
  'publicWeb.hero.tournamentEmblemAlt': 'Stemma di {name}',
  'publicWeb.hero.tournamentEmblemPlaceholderAlt': 'Nessuno stemma del torneo caricato',

  'publicWeb.bracket.roundAriaLabel': '{branch} — turno {round}',
  'publicWeb.bracket.roundHeading': 'Turno {round}',

  'publicWeb.broadcastStatus.note':
    'I risultati si aggiornano automaticamente quando c’è connessione. Altrimenti, questa pagina contiene già tutto.',

  'publicWeb.matchGrid.heading': 'Partite',
  'publicWeb.matchGrid.empty': 'Nessuna partita ancora programmata.',

  'publicWeb.series.ariaLabel': 'Serie al meglio delle {bestOf}: {home} a {away}',
  'publicWeb.series.gameWonHome': 'Partita {number}: vinta dai padroni di casa',
  'publicWeb.series.gameWonAway': 'Partita {number}: vinta dagli ospiti',
  'publicWeb.series.gameCurrent': 'Partita {number}: in corso',
  'publicWeb.series.gameUpcoming': 'Partita {number}: ancora da giocare',
  'publicWeb.series.gameNotRequired': 'Partita {number}: non si giocherà',
  'publicWeb.series.pending': 'Serie aperta sul {home}–{away}',
  'publicWeb.series.decided': '{winner} ha vinto la serie',
  'publicWeb.series.aggregate': 'Nel complesso {home}–{away}',

  'publicWeb.live.usingLastKnown': 'Visualizzazione dell’ultimo stato conosciuto.',

  'publicWeb.tournamentPage.description': 'Risultati, classifica e regolamento di {tournament}.',

  'publicWeb.livePage.title': 'In diretta',
  'publicWeb.livePage.breadcrumb': 'Percorso di navigazione',
  'publicWeb.livePage.noMatches': 'Nessuna partita in diretta',
  'publicWeb.livePage.nextKickoff': 'Prossima partita in programma',
  'publicWeb.livePage.noUpcoming': 'Nessuna partita in programma.',
  'publicWeb.livePage.seriesHeading': 'Serie',
  'publicWeb.livePage.upcomingHeading': 'Prossime',
  'publicWeb.livePage.leadersHeading': 'Leader',

  'publicWeb.bracketPage.title': 'Tabellone',

  'publicWeb.playerProfile.heading': 'Profilo Giocatore',
  'publicWeb.playerProfile.age': 'Età: {age}',
  'publicWeb.playerProfile.nationality': 'Nazionalità: {country}',
  'publicWeb.playerProfile.historyHeading': 'Storico Competizioni',
  'publicWeb.playerProfile.careerStatsHeading': 'Statistiche in Carriera',
  'publicWeb.playerProfile.noHistory': 'Nessun storico competizioni registrato.',
  'publicWeb.playerProfile.noStats': 'Nessuna statistica in carriera registrata.',
  'publicWeb.playerProfile.close': 'Chiudi',
  'publicWeb.playerProfile.backTo': '← Torna a {name}',
  'publicWeb.playerProfile.photoAlt': '{name}',
  'publicWeb.playerProfile.photoPlaceholderAlt': 'Nessuna foto caricata',
  'publicWeb.playerProfile.tournamentStatsHeading': 'Statistiche del Torneo',
  'publicWeb.playerProfile.tournamentTotalHeading': 'Totale del Torneo',
  'publicWeb.playerProfile.matchByMatchHeading': 'Partita per Partita',
  'publicWeb.playerProfile.noTournamentStats':
    'Nessuna statistica di torneo registrata per questo giocatore.',
  'publicWeb.playerProfile.matchColumnHeading': 'Partita',
  'publicWeb.playerProfile.matchLabel': 'Fase {stage} · Partita {match}',

  'publicWeb.tournamentsPage.title': 'Tornei',
  'publicWeb.tournamentsPage.liveHeading': 'Dal vivo e attivi',
  'publicWeb.tournamentsPage.upcomingHeading': 'In arrivo',
  'publicWeb.tournamentsPage.finishedHeading': 'Conclusi e archivio',
  'publicWeb.tournamentsPage.empty': 'Nessun torneo pubblicato trovato.',
  'publicWeb.tournamentsPage.champion': 'Campione',
  'publicWeb.tournamentsPage.runnerUp': 'Secondo classificato',
  'publicWeb.tournamentsPage.thirdPlace': 'Terzo posto',
  'publicWeb.tournamentsPage.viewDetails': 'Visualizza torneo',
  'publicWeb.orgPage.featuredHeading': 'In evidenza',
  'publicWeb.orgPage.clubsHeading': 'Club',
  'publicWeb.orgPage.noClubs': 'Nessun club registrato ancora.',
  'publicWeb.orgPage.emblemAlt': 'Stemma di {name}',
  'publicWeb.orgPage.emblemPlaceholderAlt': 'Nessuno stemma caricato',
  'publicWeb.orgPage.notFoundTitle': 'Organizzazione non trovata',
  'publicWeb.orgPage.notFoundBody': 'Non esiste alcuna organizzazione a questo indirizzo.',

  'publicWeb.matchesView.pageTitle': 'Partite',
  'publicWeb.matchesView.filterAll': 'Tutte',
  'publicWeb.matchesView.filterLive': 'In diretta',
  'publicWeb.matchesView.filterUpcoming': 'In programma',
  'publicWeb.matchesView.filterFinal': 'Concluse',
  'publicWeb.matchesView.empty': 'Ancora nessuna partita in questo ambito.',
  'publicWeb.matchesView.clockAriaLabel': 'Tempo trascorso: {time}',
  'publicWeb.matchesView.venueAriaLabel': 'Sede: {venue}',
  'publicWeb.matchesView.scheduledAtAriaLabel': 'Calcio d’inizio: {time}',
  'publicWeb.matchesView.latestEventAriaLabel': 'Ultimo evento: {event}',
  'publicWeb.matchesView.zoneGroupAriaLabel': 'Zona/girone: {scope}',
  'publicWeb.matchesView.stageFilterAriaLabel': 'Filtra per fase',
  'publicWeb.matchesView.zoneFilterAriaLabel': 'Filtra per zona',
  'publicWeb.matchesView.groupFilterAriaLabel': 'Filtra per girone',
  'publicWeb.matchesView.stateFilterAriaLabel': 'Filtra per stato',
  'publicWeb.matchesView.positionInGroup': '{group} — posizione n. {position}',
  'publicWeb.matchesView.position': 'Posizione n. {position}',
  'publicWeb.matchesView.decidedBy': 'Deciso da: {factor}',
  'publicWeb.matchesView.decidedByAriaLabel':
    'Il fattore decisivo tra posizioni in classifica a pari merito. Un organizzatore autorizzato può vedere la spiegazione completa nel pannello di controllo.',
  'publicWeb.matchesView.fullTraceHeading': 'Traccia completa del comparatore di classifica',
  'publicWeb.matchesView.seeAll': 'Vedi tutte le partite',

  'publicWeb.notFound.pageTitle': 'Pagina non trovata — CopaLibre',
  'publicWeb.notFound.heading': 'Pagina non trovata',
  'publicWeb.notFound.body': 'Nessun contenuto esiste a questo indirizzo.',
  'publicWeb.notFound.homeLink': 'Torna alla home',

  'publicWeb.ticker.label': 'Ticker del torneo',
  'publicWeb.ticker.leader': 'Leader',
  'publicWeb.ticker.versus': 'VS',
  'publicWeb.tvStats.homeSide': 'Casa',
  'publicWeb.tvStats.awaySide': 'Trasferta',
  'publicWeb.tvStats.points': 'Punti',
  'publicWeb.tvStats.pointsShort': 'Pti',
  'publicWeb.tvStats.unnamedActor': 'Concorrente {reference}',
  'publicWeb.tvStats.scheduledMatches': 'Partite in programma',
  'publicWeb.tvStats.status': 'Stato',
  'publicWeb.tvStats.inProgress': 'In corso',
  'publicWeb.tvStats.matchesPlayed': 'Partite giocate',
  'publicWeb.tvStats.totalScored': 'Totale realizzato',
  'publicWeb.tvStats.averagePerMatch': 'Media per partita',
  'publicWeb.tvStats.highestResult': 'Risultato più alto',
  'publicWeb.tvStats.championTitle': 'Campione del torneo',
  'publicWeb.tvStats.tableLeaderTitle': 'Leader della classifica',
  'publicWeb.tvStats.standingsRecord': '1º · {points} pti · {played} giocate',
  'publicWeb.tvStats.grandFinalRecord': 'Vincitore della finalissima ({winner} – {loser})',

  // 0223 — ticker conditions, bracket stage chrome, public header.
  'publicWeb.ticker.pause': 'Metti in pausa il ticker',
  'publicWeb.ticker.resume': 'Riprendi il ticker',
  'publicWeb.ticker.stale': 'Ultimi risultati noti — la connessione in diretta è stata persa',
  'publicWeb.ticker.empty': 'Nessuna partita da segnalare per ora',
  'publicWeb.bracket.stageAriaLabel': 'Tabellone',
  'publicWeb.bracket.zoneJumpAriaLabel': 'Vai al tabellone di una zona',
  'publicWeb.bracket.scrollAriaLabel': 'Grafico del tabellone, scorre lateralmente',
  'publicWeb.bracket.legendLabel': 'Legenda del tabellone',
  'publicWeb.bracket.outcomeAdvancing': 'Avanza',
  'publicWeb.bracket.outcomeEliminated': 'Eliminato',
  'publicWeb.bracket.outcomePending': 'Non ancora deciso',
  'publicWeb.bracket.outlineHeading': 'Turni e rami',
  'publicWeb.bracket.championshipTitle': 'Finale',
  'publicWeb.bracket.contextAriaLabel': 'Questa partita nel tabellone',
  'publicWeb.bracket.contextViewFullLink': 'Visualizza il tabellone completo',
  'publicWeb.header.openMenu': 'Apri il menu',
  'publicWeb.header.closeMenu': 'Chiudi il menu',
  'publicWeb.header.language': 'Lingua',
  'publicWeb.header.navHome': 'Home',
  'publicWeb.header.navHelp': 'Guida',
  'publicWeb.header.navApiReference': 'Riferimento API',
  'publicWeb.header.controlPanel': 'Pannello di controllo',

  // TvDashboard.tsx's own chrome (openspec 0225 task 2.6).
  'publicWeb.tvDashboard.noMatchesScheduled': 'Nessuna partita programmata al momento',
  'publicWeb.tvDashboard.standingsUnavailable': 'Classifica non disponibile',
  'publicWeb.tvDashboard.clubColumn': 'Club',
  'publicWeb.tvDashboard.playedColumn': 'PG',
  'publicWeb.tvDashboard.noTopPerformers': 'Nessun protagonista registrato',
  'publicWeb.tvDashboard.focalPanelLabel': 'Pannello principale della trasmissione',
  'publicWeb.tvDashboard.matchEventsLabel': 'Eventi della partita',
  'publicWeb.tvDashboard.statsAndTablesLabel': 'Statistiche e tabelle del torneo',
  'publicWeb.tvDashboard.sidebarSectionsLabel': 'Sezioni laterali',
  'publicWeb.tvDashboard.standingsTab': 'Classifica',
  'publicWeb.tvDashboard.performersTab': 'Migliori giocatori',
  'publicWeb.tvDashboard.statisticsTab': 'Statistiche',
  'publicWeb.completion.heading': 'Progresso del torneo',
  'publicWeb.completion.summary': '{resolved} su {total} partite giocate',
  'publicWeb.completion.complete': 'Completato',
  'publicWeb.completion.inProgress': 'In corso',
  'publicWeb.completion.unmeasured': 'Nessuna partita in programma',

  'publicWeb.matchTimeline.heading': 'Cronologia della partita',
  'publicWeb.matchTimeline.empty': 'Nessun evento disponibile al momento.',
  'publicWeb.matchOfficials.heading': 'Ufficiali di gara',
  'publicWeb.matchOfficials.scheduleNotPublished': 'L’orario non è ancora stato pubblicato.',
  'publicWeb.matchOfficials.noneAssigned': 'Nessun ufficiale di gara assegnato.',
  'publicWeb.matchRosters.heading': 'Formazioni',
  'publicWeb.matchRosters.empty': 'Le formazioni non sono ancora disponibili.',
  'publicWeb.matchRosters.teamEmpty': 'Formazione non ancora disponibile.',
  'publicWeb.matchRosters.columnNumber': 'N.',
  'publicWeb.matchRosters.columnPlayer': 'Giocatore',
  'publicWeb.matchRosters.columnRoles': 'Ruoli',
  'publicWeb.matchRosters.columnOnField': 'In campo',
  'publicWeb.matchRosters.onFieldYes': 'Sì',
  'publicWeb.matchRosters.onFieldNo': 'No',
};
