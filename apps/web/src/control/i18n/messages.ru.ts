/**
 * Russian translations, keyed to match `messages.en.ts`'s IDs exactly.
 * Best-effort machine-quality translation; native-speaker review is a
 * separate, later pass (owner's explicit choice). The plural block
 * uses Russian's four ICU categories (one/few/many/other), not English's
 * two — `react-intl`'s `Intl.PluralRules` selects the right branch for `ru`.
 */
export const messages: Record<string, string> = {
  'control.bracket.highlightEntrant': 'Выделить путь участника {entrant}',
  'control.bracket.highlightHint':
    'Выберите участника, чтобы увидеть его путь. Нажмите повторно или Escape для сброса. Будущие матчи показывают возможный путь при победах.',
  'control.shell.sections': 'Разделы',
  'control.shell.whatIsThisScreen': 'Что это за экран?',
  'control.shell.openNavigation': 'Открыть меню навигации',
  'control.shell.language': 'Язык',
  'control.shell.logout': 'Выйти',

  'control.nav.dashboard': 'Панель',
  'control.nav.liveConsole': 'Консоль прямой трансляции',
  'control.nav.tournaments': 'Турниры',
  'control.nav.roles': 'Роли',
  'control.nav.organization': 'Организация',
  'control.nav.analytics': 'Аналитика',
  'control.nav.resources': 'Площадки и судьи',
  'control.nav.auditTrail': 'Журнал аудита',
  'control.nav.platformAdministration': 'Управление платформой',
  'control.platform.sectionLabel': 'Установка',
  'control.platform.title': 'Управление платформой',
  'control.platform.description': 'Создание организаций и управление установленными модулями.',
  'control.platform.organization.heading': 'Создать организацию',
  'control.platform.organization.description':
    'Создаёт организацию и сразу приглашает первого администратора.',
  'control.platform.organization.alias': 'Псевдоним',
  'control.platform.organization.name': 'Название',
  'control.platform.organization.language': 'Основной язык',
  'control.platform.organization.timezone': 'Часовой пояс',
  'control.platform.organization.adminEmail': 'E-mail первого администратора',
  'control.platform.organization.adminRole': 'Роль первого администратора',
  'control.platform.organization.inviteAdministrator': 'Пригласить администратора',
  'control.platform.organization.create': 'Создать организацию',
  'control.platform.organization.ready':
    'Организация {alias} создана. Пригласите первого администратора.',
  'control.platform.organization.created': 'Организация {alias} создана, администратор приглашён.',
  'control.platform.users.heading': 'Администрирование пользователей',
  'control.platform.users.description':
    'Управляйте супер-админами инсталляции или перейдите в организацию, чтобы управлять её пользователями.',
  'control.platform.users.manageOrganizationAlias': 'Алиас организации',
  'control.platform.users.manageOrganizationUsers': 'Управлять пользователями',
  'control.platform.users.superAdminsHeading': 'Супер-админы инсталляции',
  'control.platform.users.noSuperAdmins': 'Супер-админов инсталляции пока нет.',
  'control.platform.users.superAdminPrincipalId': 'ID субъекта',
  'control.platform.users.createSuperAdmin': 'Назначить супер-админа',
  'control.platform.modules.heading': 'Установленные модули',
  'control.platform.modules.description':
    'Установка, удаление, проверка и поиск обновлений модулей.',
  'control.platform.modules.authorDiscipline': 'Создать дисциплину',
  'control.platform.modules.authorProfile': 'Создать профиль турнира',
  'control.platform.modules.contribute': 'Внести вклад',
  'control.platform.modules.contributed': 'Открыт пул-реквест: {url}',
  'control.platform.modules.checkUpdates': 'Проверить обновления',
  'control.platform.modules.updatesAvailable': 'Доступные обновления',
  'control.platform.modules.alias': 'Псевдоним модуля',
  'control.platform.modules.range': 'Диапазон версий (необязательно)',
  'control.platform.modules.source': 'Другой источник (одно использование)',
  'control.platform.modules.install': 'Установить модуль',
  'control.platform.modules.installed': '{alias} {version} установлен.',
  'control.platform.modules.removed': '{alias} удалён.',
  'control.platform.modules.removeConfirm': 'Удалить {alias} из этой установки?',
  'control.platform.modules.loading': 'Загрузка установленных модулей…',
  'control.platform.modules.empty': 'Нет установленных модулей.',
  'control.platform.modules.kind': 'Тип',
  'control.platform.modules.version': 'Версия',
  'control.platform.modules.sourceKind': 'Источник',
  'control.platform.modules.author': 'Автор',
  'control.platform.modules.actions': 'Действия',
  'control.platform.modules.verify': 'Проверить',
  'control.platform.modules.remove': 'Удалить',
  'control.platform.modules.verified': 'Проверено',
  'control.platform.modules.verificationPassed': '{alias} прошёл проверку.',
  'control.platform.modules.verificationFailed': '{alias} не прошёл проверку.',

  'control.lifecycle.live': 'ПРЯМОЙ ЭФИР',
  'control.lifecycle.upcoming': 'ПРЕДСТОЯЩИЙ',
  'control.lifecycle.draft': 'ЧЕРНОВИК',
  'control.lifecycle.finished': 'ЗАВЕРШЁН',

  'control.dashboard.summary': 'Сводка',
  'control.dashboard.activeTournaments': 'Активные турниры',
  'control.dashboard.pendingRegistrations': 'Ожидающие регистрации',
  'control.dashboard.matchesToday': 'Матчи сегодня',
  'control.dashboard.recentActivity': 'Недавняя активность',
  'control.dashboard.noActivityYet': 'Активности пока нет.',
  'control.dashboard.tournaments': 'Турниры',
  'control.dashboard.noTournaments': 'У этой организации пока нет турниров.',
  'control.dashboard.participantsCsv': 'CSV участников',
  'control.dashboard.resultsCsv': 'CSV результатов',
  'control.dashboard.standingsCsv': 'CSV турнирной таблицы',
  'control.dashboard.configurationJson': 'Экспорт конфигурации JSON',
  'control.dashboard.archive': 'Архивировать',

  'control.heartbeat.online': 'В сети',
  'control.heartbeat.stale': 'Нет сигнала',
  'control.heartbeat.neverSeen': 'Никогда не подключался',
  'control.heartbeat.revoked': 'Отозвано',
  'control.deviceHeartbeat.sectionLabel': 'Статус ТВ-экранов',
  'control.deviceHeartbeat.title': 'ТВ-экраны',
  'control.deviceHeartbeat.empty': 'ТВ-устройства ещё не подключены.',
  'control.deviceHeartbeat.lastSeen': 'последний сигнал',

  'control.bracket.zoomOut': 'Уменьшить',
  'control.bracket.zoomIn': 'Увеличить',
  'control.bracket.empty': 'Для этого этапа структура ещё не создана.',
  'control.bracket.groupLabel': 'Турнирная сетка',
  'control.bracket.seriesDecided': 'Решено',
  'control.bracket.seriesPending': 'Ожидает',
  'control.bracket.seriesScore': 'Серия: {home}–{away}',
  'control.bracket.seriesRemaining':
    'Осталось: {count, plural, one {игра {legs}} few {игры {legs}} many {игр {legs}} other {игры {legs}}}',
  'control.bracket.seriesAnulled':
    'Аннулировано: {count, plural, one {игра {legs}} few {игры {legs}} many {игр {legs}} other {игры {legs}}}',

  'control.mutation.blockedAfterResults':
    'Это изменение больше нельзя применить через обычное редактирование: используйте процесс проверенной корректировки.',
  'control.mutation.requiresRebuild':
    '{count, plural, =0 {Это изменение требует полной перегенерации структуры соревнования.} one {Это изменение требует перегенерировать # матч.} few {Это изменение требует перегенерировать # матча.} many {Это изменение требует перегенерировать # матчей.} other {Это изменение требует перегенерировать # матча.}}',

  'control.seeding.sectionLabel': 'Посев и турнирная сетка',
  'control.seeding.title': 'Посев',
  'control.seeding.undo': 'Отменить',
  'control.seeding.redo': 'Повторить',
  'control.seeding.randomizeUnlocked': 'Перемешать незакреплённых',
  'control.seeding.publish': 'Опубликовать посев',
  'control.seeding.order': 'Порядок посева',
  'control.seeding.toggleLockAriaLabel':
    '{locked, select, true {Освободить позицию посева {seed}} other {Закрепить позицию посева {seed}}}',
  'control.seeding.locked': 'Закреплено',
  'control.seeding.unlocked': 'Свободно',
  'control.seeding.noParticipants': 'На этом этапе нет участников.',
  'control.seeding.generatedBracket': 'Сгенерированная сетка',
  'control.seeding.unresolvedEntrant': 'Участник {id}',

  'control.report.kind.report': 'Предложенный результат',
  'control.report.kind.dispute': 'Спор',
  'control.report.genericSummary': 'Результат предложен участником — см. подробности.',
  'control.report.sectionLabel': 'Ожидающие отчёты и споры',
  'control.report.title': 'Ожидающие отчёты и споры',
  'control.report.loading': 'Загрузка отчётов...',
  'control.report.loadFailed': 'Не удалось загрузить отчёты.',
  'control.report.empty': 'Нет ожидающих отчётов или споров.',
  'control.report.attachedEvidence': 'Прикреплённое доказательство',
  'control.report.dismiss': 'Отклонить',

  'control.authoring.loadingDisciplines': 'Загрузка дисциплин...',
  'control.authoring.noDisciplines': 'Ни одна дисциплина не установлена.',
  'control.authoring.loadFailed': 'Не удалось загрузить дисциплины.',
  'control.authoring.creating': 'Создание турнира...',
  'control.authoring.created': 'Турнир создан: {alias}',
  'control.authoring.createFailed': 'Не удалось создать турнир.',

  'control.matchConsole.loading': 'Загрузка управления матчем...',
  'control.matchConsole.loadFailed': 'Не удалось загрузить официальное состояние матча.',
  'control.matchConsole.operationRejected': 'Операция была отклонена.',
  'control.matchConsole.finalizeNotConfirmed': 'Завершение не удалось подтвердить.',
  'control.matchConsole.sectionLabel': 'Управление матчем',
  'control.matchConsole.breadcrumb': '{tournamentAlias} / Матч {matchId}',
  'control.matchConsole.title': 'Операции с матчем',
  'control.matchConsole.live': 'ПРЯМОЙ ЭФИР',
  'control.matchConsole.awaitingProjection': 'Ожидание официальной проекции...',
  'control.matchConsole.syncState': 'Статус синхронизации: {state}',
  'control.matchConsole.online': 'В сети',
  'control.matchConsole.offline': 'Не в сети',
  'control.matchConsole.queuedCount':
    '{count, plural, =0 {Нет действий в очереди} one {# действие в очереди} few {# действия в очереди} many {# действий в очереди} other {# действия в очереди}}',
  'control.matchConsole.lastSynced': 'Последняя синхронизация {time}',
  'control.matchConsole.neverSynced': 'Ещё не синхронизировано',
  'control.matchConsole.unnamedEntrant': 'Участник без названия',
  'control.matchConsole.clockCommands': 'Команды таймера',
  'control.matchConsole.startClock': 'Запустить',
  'control.matchConsole.pauseClock': 'Пауза',
  'control.matchConsole.endSegment': 'Завершить период',
  'control.matchConsole.expandLedger': 'Показать всю историю',
  'control.matchConsole.collapseLedger': 'Показать меньше',
  'control.matchConsole.bracketContext': 'Место в турнирной сетке',
  'control.matchConsole.expandBracket': 'Показать сетку',
  'control.matchConsole.collapseBracket': 'Скрыть сетку',
  'control.matchConsole.refusedAction': 'Отклонено ({kind}): {reason}',
  'control.matchConsole.refusedContents': 'Вы записали: {contents}',
  'control.matchConsole.dismiss': 'Скрыть',
  'control.matchConsole.controls': 'Управление матчем',
  'control.matchConsole.currentScoreboard': 'Текущее табло',
  'control.matchConsole.clockAndPeriod': 'Таймер и период',
  'control.matchConsole.segment': 'Сегмент',
  'control.matchConsole.activeSegment': 'Активный сегмент',
  'control.matchConsole.elapsedSeconds': 'Прошедшие секунды',
  'control.matchConsole.applyClock': 'Применить таймер',
  'control.matchConsole.recordEvent': 'Зафиксировать событие',
  'control.matchConsole.participant': 'Участник',
  'control.matchConsole.eventParticipant': 'Участник события',
  'control.matchConsole.person': 'Человек',
  'control.matchConsole.eventPerson': 'Человек события',
  'control.matchConsole.staff': 'Персонал',
  'control.matchConsole.eventStaff': 'Персонал события',
  'control.matchConsole.noAttribution': 'Без привязки',
  'control.matchConsole.eventOutcome': 'Итог события',
  'control.matchConsole.description': 'Описание',
  'control.matchConsole.eventDescription': 'Описание события',
  'control.matchConsole.finalize': 'Завершить',
  'control.matchConsole.finalizeMatch': 'Завершить матч',
  'control.matchConsole.finalizeImmutable': 'Результат будет зафиксирован как неизменяемый факт.',
  'control.matchConsole.finalizeCorrections': 'Последующие исправления сохранят причину и историю.',
  'control.matchConsole.cancel': 'Отмена',
  'control.matchConsole.confirmFinalization': 'Подтвердить завершение',
  'control.matchConsole.finalizeWinnerLegend': 'Победитель',
  'control.matchConsole.finalizeNoWinner': 'Победитель не зафиксирован',
  'control.matchConsole.ledgerAndStatus': 'Журнал и статус',
  'control.matchConsole.activeTimers': 'Активные таймеры',
  'control.matchConsole.noActiveTimers': 'Нет активных таймеров.',
  'control.matchConsole.resolve': 'Разрешить',
  'control.matchConsole.eventLedger': 'Журнал событий',
  'control.matchConsole.all': 'Все',
  'control.matchConsole.categoryPositive': 'Положительные',
  'control.matchConsole.categoryNegative': 'Отрицательные',
  'control.matchConsole.categoryNeutral': 'Нейтральные',
  'control.matchConsole.segmentStatePending': 'ожидание',
  'control.matchConsole.segmentStateActive': 'активен',
  'control.matchConsole.segmentStateCompleted': 'завершён',
  'control.matchConsole.unknownSegment': 'Неизвестный сегмент',
  'control.matchConsole.clockAriaLabel': 'Таймер {time}',
  'control.matchConsole.logNote': 'Заметка в журнале',
  'control.matchConsole.jerseyGridLabel': 'Сетка футболок',
  'control.matchConsole.onField': 'На поле',
  'control.matchConsole.bench': 'Скамейка',
  'control.matchConsole.goalkeeperBadge': 'Вратарь',
  'control.matchConsole.captainBadge': 'Капитан',
  'control.matchConsole.sentOff': 'Удалён',
  'control.matchConsole.noRosterSelected': 'Состав для этого матча ещё не выбран.',
  'control.matchConsole.selectRoster': 'Выбрать состав',
  'control.matchConsole.editRoster': 'Изменить состав',
  'control.matchConsole.hideRosterStep': 'Скрыть редактор состава',
  'control.matchConsole.loadMatchData': 'Загрузить данные матча',
  'control.matchConsole.rosterStepLabel': 'Выбор состава',
  'control.matchConsole.rosterLoading': 'Загрузка кандидатов...',
  'control.matchConsole.rosterLoadFailed': 'Не удалось загрузить кандидатов.',
  'control.matchConsole.rosterSaveFailed': 'Не удалось сохранить состав.',
  'control.matchConsole.rosterSave': 'Сохранить состав',
  'control.matchConsole.rosterNumber': 'Номер игрока {name}',
  'control.matchConsole.rosterNumberPlaceholder': '№',
  'control.matchConsole.rosterNoCandidates': 'У этой команды пока нет зарегистрированных игроков.',

  'control.registration.loading': 'Загрузка регистраций...',
  'control.registration.loadFailed': 'Не удалось загрузить регистрации.',
  'control.registration.importSection': 'Импорт участников',
  'control.registration.csvLabel': 'CSV участников',
  'control.registration.importQueued': 'Проверка поставлена в очередь.',
  'control.registration.importCreateFailed': 'Не удалось создать импорт.',
  'control.registration.previewValid': 'Предпросмотр корректен.',
  'control.registration.previewInvalid': 'В предпросмотре есть ошибки.',
  'control.registration.row': 'Строка {rowNumber}: {errors}',
  'control.registration.importConfirmed': 'Импорт подтверждён.',
  'control.registration.confirmImport': 'Подтвердить импорт',
  'control.registration.contactUnavailable': 'Недоступно в этом ответе',
  'control.registration.experienceUnrecorded': 'Не зафиксировано',

  'control.review.lockExplanation':
    'Регистрация закрыта: зарегистрированные членства сохраняют право участия. Если это ошибка, исправьте её через отчёт.',
  'control.review.filter.all': 'Все',
  'control.review.filter.pending': 'Ожидающие',
  'control.review.filter.accepted': 'Принятые',
  'control.review.filter.refused': 'Отклонённые',
  'control.review.status.pending': 'Ожидает',
  'control.review.status.accepted': 'Принято',
  'control.review.status.refused': 'Отклонено',
  'control.review.status.withdrawn': 'Отозвано',
  'control.review.status.checkedIn': 'Регистрация пройдена',
  'control.review.sectionLabel': 'Проверка регистраций',
  'control.review.title': 'Проверка регистраций',
  'control.review.statusFieldLabel': 'Статус',
  'control.review.approve': 'Одобрить',
  'control.review.refuse': 'Отклонить',
  'control.review.export': 'Экспорт',
  'control.review.selectVisible': 'Выбрать видимые',
  'control.review.columnName': 'Имя',
  'control.review.columnStatus': 'Статус',
  'control.review.columnSubmitted': 'Отправлено',
  'control.review.selectRow': 'Выбрать {displayName}',
  'control.review.idLabel': 'ID: {entrantId}',
  'control.review.contact': 'Контакт',
  'control.review.teamMembers': 'Участники команды',
  'control.review.teamMembersUnavailable': 'Недоступно в этом ответе',
  'control.review.experience': 'Опыт',
  'control.review.message': 'Сообщение',
  'control.review.editMembers': 'Изменить участников',
  'control.review.editMembersTitle': 'Изменить состав команды',
  'control.review.editMembersDescription': 'Изменить роли участников команды {team}.',
  'control.review.saveMembers': 'Сохранить состав',
  'control.review.revoke': 'Отозвать',
  'control.review.emptyFilter': 'Для этого фильтра нет регистраций.',
  'control.review.pagination': 'Страница {page} из {pageCount}',
  'control.review.nationalityLabel': 'Гражданство',
  'control.review.nationalityNone': 'Не указано',
  'control.review.saveNationality': 'Сохранить',
  'control.review.uploadPhoto': 'Загрузить фото',
  'control.review.viewProfile': 'Смотреть профиль',
  'control.review.addParticipant': 'Добавить участника',
  'control.review.addParticipantTitle': 'Добавить участника',
  'control.review.addParticipantCancel': 'Отмена',
  'control.review.addParticipantSubmit': 'Зарегистрировать',
  'control.review.participantKindLabel': 'Тип',
  'control.review.participantKindPerson': 'Участник',
  'control.review.participantKindTeam': 'Команда',
  'control.review.participantNameLabel': 'Имя',
  'control.review.participantAliasLabel': 'Алиас (необязательно)',
  'control.review.editIdentity': 'Изменить',
  'control.review.editIdentityTitle': 'Изменить данные',
  'control.review.editIdentityCancel': 'Отмена',
  'control.review.editIdentitySave': 'Сохранить',
  'control.review.linkIdentity': 'Связать личность',
  'control.review.unlinkIdentity': 'Отвязать',
  'control.review.linkIdentityTitle': 'Связать {displayName} с email',
  'control.review.linkIdentityCancel': 'Отмена',
  'control.review.linkIdentitySave': 'Связать',
  'control.review.linkIdentityEmailLabel': 'Email',
  'control.abbreviationReview.sectionLabel': 'Участники, которым нужна аббревиатура',
  'control.abbreviationReview.title': 'Участники, которым нужна аббревиатура',
  'control.abbreviationReview.empty': 'У всех участников уже есть аббревиатура.',
  'control.abbreviationReview.inputLabel': 'Аббревиатура для {displayName}',
  'control.abbreviationReview.set': 'Задать',
  'control.abbreviationReview.formatError':
    'Только заглавные буквы и цифры, разделённые одиночными пробелами, до {maxLength} символов.',
  'control.abbreviationReview.setFailed': 'Не удалось задать аббревиатуру.',

  'control.country.label': 'Страна',
  'control.country.searchPlaceholder': 'Поиск страны…',

  'control.personProfile.title': 'Профиль участника',
  'control.personProfile.loading': 'Загрузка профиля…',
  'control.personProfile.loadFailed': 'Не удалось загрузить этот профиль.',
  'control.personProfile.photoAlt': 'Фото {displayName}',
  'control.personProfile.photoPlaceholderAlt': 'Фото не загружено',
  'control.personProfile.naturalKeyLabel': 'Документ',
  'control.personProfile.naturalKeyUnavailable': 'Не указан',
  'control.personProfile.nationalityLabel': 'Гражданство',
  'control.personProfile.back': 'Назад к заявкам',

  'control.jerseyGrid.emblemAlt': 'Эмблема клуба',
  'control.jerseyGrid.emblemPlaceholderAlt': 'Нет эмблемы',

  'control.roles.loadFailed': 'Не удалось загрузить пользователей.',
  'control.roles.changeFailed': 'Не удалось применить изменение.',
  'control.roles.sectionLabel': 'Роли и права',
  'control.roles.breadcrumb': '{organizationAlias} / Организация',
  'control.roles.title': 'Роли и права',
  'control.roles.addRecipient': 'Добавить получателя',
  'control.roles.columnUser': 'Пользователь',
  'control.roles.columnRole': 'Роль',
  'control.roles.columnStatus': 'Статус',
  'control.roles.columnActions': 'Действия',
  'control.roles.loading': 'Загрузка пользователей...',
  'control.roles.empty': 'Пользователи не назначены.',
  'control.roles.roleOf': 'Роль {email}',
  'control.roles.statusOf': 'Статус {email}',
  'control.roles.active': 'Активен',
  'control.roles.inactive': 'Неактивен',
  'control.roles.deleteOf': 'Удалить {email}',
  'control.roles.delete': 'Удалить',
  'control.roles.pendingInvitationsTitle': 'Ожидающие приглашения',
  'control.roles.pendingInvitationsEmpty': 'Нет ожидающих приглашений.',
  'control.roles.rescindInvitation': 'Отозвать',
  'control.roles.rescindInvitationOf': 'Отозвать приглашение для {email}',
  'control.roles.lastActiveAdminNotice':
    'В организации всегда должен быть хотя бы один активный админ.',
  'control.roles.role.admin': 'Админ',
  'control.roles.role.clubAdmin': 'Админ клуба',
  'control.roles.role.tournamentAdmin': 'Админ турнира',
  'control.roles.role.referee': 'Судья',
  'control.roles.role.broadcaster': 'Трансляция',
  'control.roles.role.viewer': 'Наблюдатель',
  'control.roles.description.admin':
    'Управляет всем, что делает организация: создаёт и публикует турниры, управляет каждым пользователем, администрирует каждый клуб и проводит матчи.',
  'control.roles.description.clubAdmin':
    'Управляет данными одного клуба (название, алиас, аббревиатура и эмблема) без доступа ко всей организации.',
  'control.roles.description.tournamentAdmin':
    'Ведёт один турнир от начала до конца, без охвата остальной части организации.',
  'control.roles.description.referee':
    'Управляет матчем в реальном времени: фиксирует события, управляет таймером матча, разрешает таймеры и выбирает состав.',
  'control.roles.description.broadcaster':
    'Обозначает того, кто занимается трансляцией турнира, а не его администрированием; сегодня не предоставляет никаких операционных прав.',
  'control.roles.description.viewer':
    'Роль организации с наименьшими правами: принадлежит организации, не предоставляя никаких операционных прав.',
  'control.roles.learnMore': 'Подробнее',
  'control.roles.inviteDialog.close': 'Закрыть',
  'control.registration.modalClose': 'Закрыть',
  'control.shell.drawerClose': 'Закрыть меню',
  'control.roles.inviteDialog.email': 'Эл. почта',
  'control.roles.inviteDialog.role': 'Роль',
  'control.roles.inviteDialog.roleAriaLabel': 'Роль приглашения',
  'control.roles.inviteDialog.activeOnAccept': 'Активен после принятия',
  'control.roles.inviteDialog.club': 'Клуб',
  'control.roles.inviteDialog.clubAriaLabel': 'Администрируемый клуб',
  'control.roles.inviteDialog.tournament': 'Турнир',
  'control.roles.inviteDialog.tournamentAriaLabel': 'Администрируемый турнир',
  'control.roles.inviteDialog.cancel': 'Отмена',
  'control.roles.inviteDialog.submit': 'Отправить приглашение',
  'control.roles.selectClubPlaceholder': 'Выберите клуб…',
  'control.roles.selectTournamentPlaceholder': 'Выберите турнир…',

  'control.auditTrail.title': 'Журнал аудита',
  'control.auditTrail.description':
    'Что произошло в этой организации: применённые изменения и отклонённые попытки — от новых к старым.',
  'control.auditTrail.breadcrumb': '{organizationAlias} / Организация',
  'control.auditTrail.loading': 'Загрузка журнала...',
  'control.auditTrail.empty': 'Пока ничего не записано.',
  'control.auditTrail.loadFailed': 'Не удалось загрузить журнал аудита.',
  'control.auditTrail.columnTime': 'Когда',
  'control.auditTrail.columnActor': 'Кто',
  'control.auditTrail.columnAction': 'Действие',
  'control.auditTrail.columnOutcome': 'Результат',
  'control.auditTrail.columnReason': 'Причина',
  'control.auditTrail.outcome.applied': 'Применено',
  'control.auditTrail.outcome.refused': 'Отклонено',
  'control.auditTrail.actorFilterLabel': 'Фильтр по пользователю',
  'control.auditTrail.actorFilterClear': 'Сбросить фильтр',
  'control.auditTrail.previousPage': 'Назад',
  'control.auditTrail.nextPage': 'Далее',
  'control.auditTrail.pageStatus': '{start}–{end} из {total}',

  'control.wizard.step.name': 'Название',
  'control.wizard.step.discipline': 'Дисциплина',
  'control.wizard.step.format': 'Формат',
  'control.wizard.step.ruleset': 'Правила дисциплины',
  'control.wizard.ruleset.empty':
    'Эта дисциплина не объявляет дополнительных правил для настройки здесь.',
  'control.wizard.step.window': 'Окно',
  'control.wizard.step.summary': 'Сводка',
  'control.wizard.formatPreview.title': 'Предварительный просмотр структуры',
  'control.wizard.formatPreview.illustrative': 'Иллюстративный просмотр (8 участников)',
  'control.wizard.formatPreview.capacity':
    '{count, plural, one {# участник} few {# участника} other {# участников}}',

  'control.wizard.problem.missingName': 'Отсутствует название',
  'control.wizard.problem.aliasFormat': 'Алиас использует строчные буквы и дефисы',
  'control.wizard.problem.chooseDiscipline': 'Выберите дисциплину',
  'control.wizard.problem.chooseFormat': 'Выберите формат',
  'control.wizard.problem.formatNotSupported': 'Выбранная дисциплина не поддерживает этот формат',
  'control.wizard.problem.minParticipants': 'Турниру нужно как минимум два участника',
  'control.wizard.problem.seriesOnPlacementFormat':
    'Этот формат ранжирует всех участников, а не сводит двоих, поэтому он не может решаться серией',
  'control.wizard.problem.seriesSpan': 'Серия включает минимум два матча',
  'control.wizard.problem.seriesEvenBestOf':
    'Серии до N побед нужно нечётное число матчей, чтобы одна сторона набрала большинство. Для чётного числа используйте По сумме или Очки за матч.',
  'control.wizard.problem.allocationAttributeKey':
    'Посев по весу требует атрибут для сортировки участников',
  'control.stageEditor.title': 'Этапы',
  'control.stageEditor.addStage': 'Добавить этап',
  'control.stageEditor.removeStage': 'Удалить',
  'control.stageEditor.stageHeading': 'Этап {number}',
  'control.stageEditor.stageName': 'Название этапа',
  'control.stageEditor.stageFormat': 'Формат этапа',
  'control.stageEditor.seriesToggle': 'Определять пары этого этапа серией матчей',
  'control.stageEditor.seriesSpan': 'Матчи',
  'control.stageEditor.seriesResolutionClass': 'Тип решения',
  'control.stageEditor.seriesNeutralGround': 'Нейтральное поле',
  'control.stageEditor.seriesAccountPerSeries': 'Учитывать таблицу по сериям, а не по матчам',
  'control.stageEditor.allocationLabel': 'Посев',
  'control.stageEditor.allocationNone': 'Решить позже',
  'control.stageEditor.allocationAutomatic': 'Автоматически — порядок отбора предыдущего этапа',
  'control.stageEditor.allocationManual': 'Вручную — я сам расставлю участников',
  'control.stageEditor.allocationWeighted': 'По весу — сортировать по атрибуту участника',
  'control.stageEditor.allocationAttributeKey': 'Атрибут',
  'control.stageEditor.allocationDirection': 'Направление',
  'control.stageEditor.allocationDirectionHigherFirst': 'Сначала более высокое значение',
  'control.stageEditor.allocationDirectionLowerFirst': 'Сначала более низкое значение',
  'control.stageEditor.profilePreviewHint':
    'Этот турнир будет использовать собственные этапы профиля, показанные здесь только для чтения.',
  'control.wizard.series.enable': 'Решать каждую пару серией матчей',
  'control.wizard.series.help':
    'Оставьте выключенным, чтобы играть один матч на пару. При включении между теми же двумя сторонами назначается несколько матчей, и пара решается по их совокупности.',
  'control.wizard.field.seriesSpan': 'Матчей в серии',
  'control.wizard.field.seriesResolutionClass': 'Решается по',
  'control.wizard.series.class.bestOf': 'До N побед — первая сторона, набравшая большинство',
  'control.wizard.series.class.aggregate': 'По сумме — счёт всех матчей складывается',
  'control.wizard.series.class.pointsPerLeg': 'Очки за матч — каждый матч приносит очки',
  'control.wizard.field.seriesNeutralGround': 'Играется на нейтральном поле (без дома и гостей)',
  'control.wizard.field.seriesStandingsAccounting': 'Засчитывается в таблицу как',
  'control.wizard.series.accounting.match':
    'Один результат за матч — каждый матч серии добавляет в таблицу свою победу, ничью или поражение',
  'control.wizard.series.accounting.series':
    'Один результат за серию — вся серия добавляет в таблицу одну победу, ничью или поражение, независимо от числа сыгранных матчей',

  'control.wizard.breadcrumb': 'Турниры > Новый',
  'control.wizard.title': 'Создать турнир',
  'control.wizard.configured': 'настроено',
  'control.wizard.steps': 'Шаги',
  'control.wizard.field.name': 'Название',
  'control.wizard.field.alias': 'Алиас',
  'control.wizard.field.discipline': 'Дисциплина',
  'control.wizard.field.format': 'Формат',
  'control.wizard.field.region': 'Регион',
  'control.wizard.field.capacity': 'Вместимость',
  'control.wizard.field.profile': 'Профиль соревнований',
  'control.wizard.profile.none': 'Нет (Одиночный этап)',
  'control.wizard.field.checkInClosesAt': 'Дедлайн регистрации',
  'control.wizard.publicRegistration': 'Открыть публичную регистрацию',
  'control.wizard.requiresCheckIn': 'Требуется регистрация на месте',
  'control.wizard.back': 'Назад',
  'control.wizard.create': 'Создать турнир',
  'control.wizard.continue': 'Продолжить',

  'control.wizard.decision.discipline':
    'Определяет правила, статистику и события, доступные для этого соревнования.',
  'control.wizard.decision.format':
    'Определяет, как формируются встречи и как участники проходят дальше.',
  'control.wizard.decision.seriesSpan':
    'Сколько матчей может включать серия; то, как они складываются в единого победителя, определяется ниже.',
  'control.wizard.decision.seriesResolutionClass':
    'Определяет, как матчи серии складываются в единый результат.',
  'control.wizard.decision.seriesNeutralGround':
    'Отмечает серию как сыгранную на нейтральном поле, поэтому ни одна из сторон не считается хозяином.',
  'control.wizard.decision.seriesStandingsAccounting':
    'Определяет, учитывает ли таблица каждый матч серии отдельно или всю серию как единый результат.',
  'control.wizard.decision.region':
    'Группирует этот турнир для региональной фильтрации в публичных списках; не ограничивает, где проходят матчи.',
  'control.wizard.decision.capacity': 'Максимальное число участников, которое примет турнир.',
  'control.wizard.decision.publicRegistration':
    'Позволяет участникам регистрироваться самостоятельно, а не только через организатора.',
  'control.wizard.decision.requiresCheckIn':
    'Требует, чтобы каждый участник прошёл регистрацию перед началом турнира, иначе он будет отмечен как отсутствующий.',
  'control.wizard.decision.checkInClosesAt':
    'Момент закрытия регистрации; участник, не прошедший её к этому времени, отмечается как отсутствующий.',
  'control.wizard.format.singleElimination.description':
    'Одно поражение выбывает участника из турнирной сетки.',
  'control.wizard.format.doubleElimination.description':
    'Участник выбывает только после двух поражений — через сетку победителей и сетку проигравших.',
  'control.wizard.format.roundRobin.description':
    'Каждый участник играет с каждым другим один раз; таблица упорядочена по набранным очкам.',
  'control.wizard.format.league.description':
    'Каждый участник играет с каждым другим по расписанию сезона; таблица упорядочена по набранным очкам.',
  'control.wizard.format.roundRobinSingleLeg.description':
    'Каждый участник играет с каждым другим ровно один раз, без ответного матча.',
  'control.wizard.format.roundRobinHomeAway.description':
    'Каждый участник играет с каждым другим дважды, по разу на своём поле.',
  'control.wizard.format.freeForAll.description':
    'Все участники соревнуются одновременно в одном заезде; таблица упорядочена по месту финиша.',
  'control.wizard.format.heats.description':
    'Участники соревнуются в нескольких заездах; таблица упорядочена по месту финиша по всем заездам.',
  'control.wizard.series.class.bestOf.description':
    'Серия завершается, как только одна из сторон выигрывает достаточно матчей, чтобы оставшиеся матчи стали неважны.',
  'control.wizard.series.class.aggregate.description':
    'Победитель серии определяется по суммарному счёту всех матчей, а не по числу выигранных матчей.',
  'control.wizard.series.class.pointsPerLeg.description':
    'Каждый матч серии приносит свои очки; победителем серии становится тот, кто набрал больше всего очков в сумме.',
  'control.wizard.series.accounting.match.description':
    'Таблица учитывает каждый матч серии как отдельный результат.',
  'control.wizard.series.accounting.series.description':
    'Таблица учитывает всю серию как единый результат, а не каждый матч по отдельности.',
  'control.wizard.mutation.requiresRebuild':
    'Изменение этого параметра после формирования встреч аннулирует их и создаёт заново.',
  'control.wizard.mutation.blockedAfterResults':
    'Это нельзя изменить после появления результата; вместо этого используйте процедуру аудируемого исправления.',

  'control.standings.sectionLabel': 'Турнирная таблица',
  'control.standings.title': 'Турнирная таблица',
  'control.standings.projectionVersion': 'Проекция v{version}',
  'control.standings.unresolvedTie': ' · неразрешённая ничья',
  'control.standings.participant': 'Участник',
  'control.standings.tiebreak': 'Тай-брейк',
  'control.standings.noTiebreak': 'На этой позиции не применялся ни один компаратор тай-брейка.',
  'control.standings.noResultsYet': 'На этом этапе пока нет результатов.',
  'control.standings.traceFetchFailed': 'Не удалось получить трассировку движка правил.',
  'control.standings.traceLoading': 'Получение трассировки…',
  'control.standings.traceOpenRow': 'Откройте строку, чтобы увидеть трассировку.',
  'control.standings.traceNoComparators': 'Движок не зафиксировал ни одного компаратора.',
  'control.standings.traceAriaLabel': 'Трассировка тай-брейка',
  'control.standings.traceTitle': 'Трассировка тай-брейка',
  'control.standings.distributionAriaLabel': 'Распределение очков',
  'control.standings.distributionTitle': 'Распределение очков · Топ {count}',
  'control.standings.distributionEmpty': 'Нет данных для графика.',
  'control.standings.sharedRank': 'Общая позиция',
  'control.standings.tieBroken': 'Тай-брейк применён',
  'control.standings.exportCsv': 'Экспорт в CSV',
  'control.standings.noLayouts': 'Эта дисциплина не объявляет таблиц для этого турнира.',
  'control.standings.groupSelector': 'Группа',
  'control.standings.grain.series': 'Эта таблица засчитывает один результат за серию.',
  'control.standings.grain.match': 'Эта таблица засчитывает один результат за сыгранный матч.',
  'control.standings.column.series.label': 'Серии',
  'control.standings.column.series.shortLabel': 'С',
  'control.zoneGroup.loading': 'Загрузка…',
  'control.zoneGroup.loadFailed': 'Не удалось загрузить зоны и группы.',
  'control.zoneGroup.saveFailed': 'Запрос отклонён.',
  'control.zoneGroup.sectionLabel': 'Зоны и группы',
  'control.zoneGroup.breadcrumb': '{tournamentAlias} · Этап {stageNumber}',
  'control.zoneGroup.title': 'Зоны и группы',
  'control.zoneGroup.zonesHeading': 'Зоны',
  'control.zoneGroup.newZoneName': 'Название новой зоны',
  'control.zoneGroup.addZone': 'Добавить зону',
  'control.zoneGroup.assignZonesHeading': 'Распределить участников по зонам',
  'control.zoneGroup.automaticDraw': 'Автоматическая жеребьёвка',
  'control.zoneGroup.manualPlacement': 'Ручное размещение',
  'control.zoneGroup.zoneCount': 'Количество зон',
  'control.zoneGroup.groupCount': 'Количество групп',
  'control.zoneGroup.seed': 'Сид жеребьёвки',
  'control.zoneGroup.previewDraw': 'Предпросмотр жеребьёвки',
  'control.zoneGroup.confirmDraw': 'Подтвердить жеребьёвку',
  'control.zoneGroup.placementNumber': '{name} — номер',
  'control.zoneGroup.saveAssignment': 'Сохранить распределение',
  'control.zoneGroup.previewResult': 'Предпросмотр готов — распределено участников: {count}.',
  'control.zoneGroup.assignmentSaved': 'Распределение сохранено.',
  'control.zoneGroup.selectZone': 'Зона',
  'control.zoneGroup.groupsHeading': 'Группы',
  'control.zoneGroup.newGroupName': 'Название новой группы',
  'control.zoneGroup.addGroup': 'Добавить группу',
  'control.zoneGroup.renameZoneLabel': 'Переименовать зону {name}',
  'control.zoneGroup.renameGroupLabel': 'Переименовать группу {name}',
  'control.zoneGroup.rename': 'Переименовать',
  'control.zoneGroup.delete': 'Удалить',
  'control.zoneGroup.noEntrantsAssigned': 'Участники ещё не распределены',
  'control.settings.title': 'Настройки турнира',
  'control.settings.loading': 'Загрузка настроек…',
  'control.settings.loadFailed': 'Не удалось загрузить настройки турнира.',
  'control.settings.nameLabel': 'Название',
  'control.settings.regionLabel': 'Регион',
  'control.settings.capacityLabel': 'Вместимость',
  'control.settings.checkInClosesAtLabel': 'Окончание регистрации',
  'control.settings.featuredLabel': 'Выделить на публичной странице организации',
  'control.settings.preview': 'Предпросмотр',
  'control.settings.save': 'Сохранить',
  'control.settings.saved': 'Настройки сохранены.',
  'control.settings.mutationClass':
    '{mutationClass, select, safe {Безопасно} requires_rebuild {Требует пересборки} blocked_after_results {Заблокировано после результатов} other {Неизвестно}}',
  'control.settings.link': 'Настройки турнира',
  'control.settings.emblemHeading': 'Эмблема турнира',
  'control.settings.emblemAlt': 'Эмблема турнира',
  'control.settings.emblemPlaceholderAlt': 'Эмблема турнира не загружена',
  'control.settings.uploadEmblem': 'Загрузить эмблему',
  'control.settings.removeEmblem': 'Удалить эмблему',
  'control.settings.emblemUploaded': 'Эмблема турнира загружена.',
  'control.settings.emblemRemoved': 'Эмблема турнира удалена.',
  'control.rulesetOverrides.title': 'Регламент',
  'control.rulesetOverrides.link': 'Регламент',
  'control.rulesetOverrides.fields': 'Поля переопределения',
  'control.rulesetOverrides.removeField': 'Удалить',
  'control.rulesetOverrides.addField': 'Добавить поле',
  'control.rulesetOverrides.newFieldLabel': 'Поле (путь)',
  'control.rulesetOverrides.newFieldValueLabel': 'Значение (JSON)',
  'control.stageSettings.title': 'Настройки этапа',
  'control.stageSettings.renameLabel': 'Новое название этапа',
  'control.stageSettings.renameSubmit': 'Переименовать',
  'control.stageSettings.formatLabel': 'Формат',
  'control.stageSettings.formatSubmit': 'Изменить формат',
  'control.stageSettings.delete': 'Удалить этап',
  'control.stageSettings.seededExplanation':
    'На этом этапе уже сформированы матчи, поэтому формат и удаление заблокированы.',
  'control.tournamentHub.title': 'Этапы',
  'control.tournamentHub.stagesHeading': 'Этапы',
  'control.tournamentHub.empty': 'В этом турнире пока нет этапов.',
  'control.tournamentHub.seeded': 'С посевом',
  'control.tournamentHub.unseeded': 'Без посева',
  'control.tournamentHub.openStage': 'Открыть этап {number}: {name}',
  'control.tournamentHub.loading': 'Загрузка этапов…',
  'control.tournamentHub.loadFailed': 'Не удалось загрузить этапы этого турнира.',
  'control.stageHub.title': 'Этап {number}',
  'control.stageHub.toolsHeading': 'Инструменты этапа',
  'control.stageHub.seedingLink': 'Посев',
  'control.stageHub.zoneGroupsLink': 'Зоны и группы',
  'control.stageHub.standingsLink': 'Турнирная таблица',
  'control.stageHub.scheduleLink': 'Расписание',
  'control.stageHub.loading': 'Загрузка этапа…',
  'control.stageHub.loadFailed': 'Не удалось загрузить этот этап.',
  'control.stageHub.notFound': 'Нет этапа {number} в этом турнире.',
  'control.stageHub.breadcrumbLink': 'Этап {stageNumber}',
  'control.dashboard.stages': 'Этапы',
  'control.stageConfiguration.title': 'Конфигурация этапа',
  'control.stageConfiguration.fieldLabel': 'Поле (путь)',
  'control.stageConfiguration.valueLabel': 'Значение (JSON)',
  'control.stageConfiguration.apply': 'Применить',
  'control.stageConfiguration.saved': 'Конфигурация этапа сохранена.',
  'control.stageConfiguration.seededExplanation':
    'На этом этапе уже сформированы матчи, поэтому его конфигурация заблокирована.',
  'control.zoneGroup.assignGroupsHeading': 'Распределить участников по группам',
  'control.zoneGroup.openPromotionPlan': 'Открыть план повышения',
  'control.promotion.loading': 'Загрузка…',
  'control.promotion.sectionLabel': 'План повышения',
  'control.promotion.breadcrumb': '{tournamentAlias} · {zoneName}',
  'control.promotion.title': 'План повышения',
  'control.promotion.configHeading': 'Настройка',
  'control.promotion.nextStageNumber': 'Номер следующего этапа',
  'control.promotion.perGroupAdvance': 'Количество участников, проходящих из каждой группы',
  'control.promotion.bandsHeading': 'Полосы (только если на следующем этапе несколько зон)',
  'control.promotion.bandZoneRef': 'Название зоны назначения',
  'control.promotion.bandCount': 'Количество',
  'control.promotion.removeBand': 'Удалить полосу',
  'control.promotion.addBand': 'Добавить полосу',
  'control.promotion.savePlan': 'Сохранить план повышения',
  'control.promotion.reviewHeading': 'Проверка — рассчитанный порядок кандидатов',
  'control.promotion.planSaved': 'План сохранён.',
  'control.promotion.saveFailed': 'План отклонён.',
  'control.promotion.noPlanYet': 'Для этой зоны ещё не сохранён план повышения.',

  'control.landing.emptyTitle': 'Пока нет организаций',
  'control.landing.emptyBody':
    'У этой учётной записи пока нет роли ни в одной организации. Требуется приглашение от администратора.',
  'control.landing.pickerTitle': 'Выберите организацию',
  'control.landing.loading': 'Загрузка панели управления…',
  'control.landing.errorTitle': 'Ошибка загрузки организаций',
  'control.landing.errorGeneric': 'Не удалось загрузить организацию',
  'control.landing.backToLogin': 'Вернуться к входу',
  'control.notFound.title': 'Экран не найден',
  'control.notFound.body': 'Нет экрана управления для {path}.',
  'control.callback.loading': 'Завершение входа…',
  'control.callback.errorTitle': 'Не удалось завершить вход',
  'control.callback.backHome': 'Вернуться на главную',

  'control.loadMatchData.title': 'Загрузить данные матча',
  'control.loadMatchData.breadcrumb': '{tournamentAlias} · Матч {matchId}',
  'control.loadMatchData.sectionLabel': 'Загрузить данные матча',
  'control.loadMatchData.loading': 'Загрузка…',
  'control.loadMatchData.loadFailed': 'Не удалось загрузить этот матч.',
  'control.loadMatchData.forbidden':
    'У вас нет прав, необходимых для этого экрана применительно к этому матчу.',
  'control.loadMatchData.notScheduled':
    'В этом матче уже есть зарегистрированная активность или результат. Используйте вместо этого живую консоль — загрузка данных здесь создаст вторую, конфликтующую историю событий.',
  'control.loadMatchData.rosterHeading': 'Состав',
  'control.loadMatchData.rosterCandidatesLoading': 'Загрузка кандидатов в состав…',
  'control.loadMatchData.segmentsHeading': 'Сегменты',
  'control.loadMatchData.addSegment': 'Добавить сегмент',
  'control.loadMatchData.removeSegment': 'Удалить сегмент',
  'control.loadMatchData.segmentType': 'Тип',
  'control.loadMatchData.segmentTypePlaceholder': 'напр. тайм, период, сет',
  'control.loadMatchData.segmentElapsedSeconds': 'Продолжительность (секунды)',
  'control.loadMatchData.eventsHeading': 'События',
  'control.loadMatchData.addEvent': 'Добавить событие',
  'control.loadMatchData.removeEvent': 'Удалить событие',
  'control.loadMatchData.moveEventUp': 'Переместить вверх',
  'control.loadMatchData.moveEventDown': 'Переместить вниз',
  'control.loadMatchData.eventDefinition': 'Событие',
  'control.loadMatchData.eventSegment': 'Сегмент',
  'control.loadMatchData.eventOccurredAt': 'Когда',
  'control.loadMatchData.eventSide': 'Сторона',
  'control.loadMatchData.eventPerson': 'Человек',
  'control.loadMatchData.eventNoAttribution': 'Без указания',
  'control.loadMatchData.eventNotes': 'Заметки',
  'control.loadMatchData.noEvents': 'Событий пока нет. Добавьте одно или импортируйте CSV ниже.',
  'control.loadMatchData.resultHeading': 'Результат',
  'control.loadMatchData.winner': 'Победитель',
  'control.loadMatchData.noWinnerDraw': 'Без победителя / ничья',
  'control.loadMatchData.submit': 'Отправить данные матча',
  'control.loadMatchData.submitting': 'Отправка…',
  'control.loadMatchData.submitSucceeded':
    'Матч загружен и завершён — зарегистрировано событий: {eventCount}.',
  'control.loadMatchData.submitFailed':
    'Отправка отклонена. Ничего не было записано — исправьте запись ниже и отправьте снова.',
  'control.loadMatchData.backToConsole': 'Открыть консоль матча',
  'control.loadMatchData.csvHeading': 'Импорт из CSV',
  'control.loadMatchData.csvHelp':
    'Загружает таблицу в редактор выше для проверки — ничего не отправляется, пока вы сами не проверите и не нажмёте «Отправить данные матча».',
  'control.loadMatchData.csvChooseFile': 'Выбрать файл CSV',
  'control.loadMatchData.csvDownloadTemplate': 'Скачать шаблон',
  'control.loadMatchData.csvErrorsHeading':
    'Найдено проблем: {count} — исправьте их в таблице и импортируйте снова.',
  'control.loadMatchData.csvLoaded': 'CSV загружен в редактор ниже для проверки.',
  'control.clubManagement.sectionLabel': 'Управление клубами',
  'control.clubManagement.title': 'Клубы',
  'control.clubManagement.loading': 'Загрузка…',
  'control.clubManagement.loadFailed': 'Не удалось загрузить клубы.',
  'control.clubManagement.saveFailed': 'Запрос отклонён.',
  'control.clubManagement.empty': 'В этой организации пока нет клубов.',
  'control.clubManagement.edit': 'Изменить',
  'control.clubManagement.newClubName': 'Название нового клуба',
  'control.clubManagement.newClubAlias': 'Алиас (необязательно)',
  'control.clubManagement.newClubAbbreviation': 'Сокращение (необязательно)',
  'control.clubManagement.addClub': 'Добавить клуб',
  'control.clubManagement.editHeading': 'Изменить клуб',
  'control.clubManagement.name': 'Название',
  'control.clubManagement.alias': 'Алиас',
  'control.clubManagement.abbreviation': 'Сокращение',
  'control.clubManagement.saveChanges': 'Сохранить изменения',
  'control.clubManagement.uploadEmblem': 'Загрузить эмблему',
  'control.clubManagement.emblemAlt': 'Эмблема {name}',
  'control.clubManagement.emblemPlaceholderAlt': 'Эмблема не загружена',
  'control.clubManagement.saved': 'Изменения сохранены.',
  'control.clubManagement.created': 'Клуб создан.',
  'control.clubManagement.emblemUploaded': 'Эмблема загружена.',
  'control.resourceManagement.sectionLabel': 'Управление площадками и судьями',
  'control.resourceManagement.title': 'Площадки и судьи',
  'control.resourceManagement.loading': 'Загрузка…',
  'control.resourceManagement.loadFailed': 'Не удалось загрузить площадки и судей.',
  'control.resourceManagement.saveFailed': 'Запрос отклонён.',
  'control.resourceManagement.venuesHeading': 'Площадки',
  'control.resourceManagement.venuesEmpty': 'У этой организации пока нет площадок.',
  'control.resourceManagement.officialsHeading': 'Судьи',
  'control.resourceManagement.officialsEmpty': 'У этой организации пока нет судей.',
  'control.resourceManagement.edit': 'Изменить',
  'control.resourceManagement.newVenueName': 'Название новой площадки',
  'control.resourceManagement.newVenueAlias': 'Алиас',
  'control.resourceManagement.newVenueCapacity': 'Одновременная вместимость',
  'control.resourceManagement.addVenue': 'Добавить площадку',
  'control.resourceManagement.editVenueHeading': 'Изменить площадку',
  'control.resourceManagement.venueName': 'Название',
  'control.resourceManagement.venueCapacity': 'Одновременная вместимость',
  'control.resourceManagement.venueAddress': 'Адрес (необязательно)',
  'control.resourceManagement.detailsHeading': 'Детали',
  'control.resourceManagement.detailsHint':
    'Произвольно: адрес, тип покрытия, адрес сервера, регион, текущая карта — всё, что нужно ' +
    'зафиксировать для этой площадки. Никогда не проверяется.',
  'control.resourceManagement.detailKey': 'Название детали',
  'control.resourceManagement.detailValue': 'Значение',
  'control.resourceManagement.addDetail': 'Добавить деталь',
  'control.resourceManagement.removeDetail': 'Удалить',
  'control.resourceManagement.saveVenueChanges': 'Сохранить площадку',
  'control.resourceManagement.venueSaved': 'Площадка сохранена.',
  'control.resourceManagement.venueCreated': 'Площадка создана.',
  'control.resourceManagement.newOfficialName': 'Имя нового судьи',
  'control.resourceManagement.addOfficial': 'Добавить судью',
  'control.resourceManagement.editOfficialHeading': 'Изменить судью',
  'control.resourceManagement.officialName': 'Имя',
  'control.resourceManagement.officialRoles': 'Роли',
  'control.resourceManagement.saveOfficialChanges': 'Сохранить судью',
  'control.resourceManagement.officialSaved': 'Судья сохранён.',
  'control.resourceManagement.officialCreated': 'Судья создан.',
  'control.resourceManagement.roleReferee': 'Главный судья',
  'control.resourceManagement.roleAssistant': 'Помощник судьи',
  'control.resourceManagement.roleTableOfficial': 'Судья за столиком',
  'control.resourceManagement.roleObserver': 'Наблюдатель',
  'control.resourceManagement.schedulesHeading': 'Расписания',
  'control.resourceManagement.schedulesEmpty': 'У этой организации пока нет расписаний.',
  'control.resourceManagement.newScheduleName': 'Название расписания',
  'control.resourceManagement.newScheduleStartsAt': 'Начало',
  'control.resourceManagement.newScheduleEndsAt': 'Окончание',
  'control.resourceManagement.newScheduleSlotMinutes': 'Минут на слот',
  'control.resourceManagement.newScheduleTurnaroundMinutes': 'Минут перерыва',
  'control.resourceManagement.newScheduleVenues': 'Площадки',
  'control.resourceManagement.addSchedule': 'Добавить расписание',
  'control.resourceManagement.scheduleCreated': 'Расписание создано.',
  'control.resourceManagement.scheduleSaved': 'Расписание сохранено.',
  'control.resourceManagement.scheduleDeleted': 'Расписание удалено.',
  'control.resourceManagement.editScheduleHeading': 'Редактировать расписание',
  'control.resourceManagement.scheduleSlotsCount': 'Сгенерировано слотов: {count}',
  'control.resourceManagement.saveScheduleChanges': 'Сохранить расписание',
  'control.resourceManagement.deleteSchedule': 'Удалить расписание',
  'control.scheduleBuilder.sectionLabel': 'Конструктор расписания',
  'control.scheduleBuilder.title': 'Расписание',
  'control.scheduleBuilder.loading': 'Загрузка…',
  'control.scheduleBuilder.loadFailed': 'Не удалось загрузить расписание этого этапа.',
  'control.scheduleBuilder.saveFailed': 'Запрос отклонён.',
  'control.scheduleBuilder.calendarViewLabel': 'Вид календаря',
  'control.scheduleBuilder.listViewLabel': 'Вид списка',
  'control.scheduleBuilder.noFixtures': 'На этом этапе пока нет сформированных матчей.',
  'control.scheduleBuilder.unassigned': 'Не назначено',
  'control.scheduleBuilder.fixtureRound': 'Раунд {round}',
  'control.scheduleBuilder.startTime': 'Время начала',
  'control.scheduleBuilder.duration': 'Длительность (минуты)',
  'control.scheduleBuilder.venue': 'Площадка',
  'control.scheduleBuilder.noVenue': 'Без площадки',
  'control.scheduleBuilder.officials': 'Судьи',
  'control.scheduleBuilder.preview': 'Предпросмотр',
  'control.scheduleBuilder.publish': 'Опубликовать',
  'control.scheduleBuilder.published': 'Расписание опубликовано.',
  'control.scheduleBuilder.conflictsHeading': 'Конфликты',
  'control.scheduleBuilder.affectedPublishedHeading':
    'Этот пакет переместит уже опубликованный матч',
  'control.scheduleBuilder.dayOff': 'В этом диапазоне матчей не запланировано',
  'control.scheduleBuilder.entrantColumn': 'Участник',
  'control.scheduleBuilder.timeColumn': 'Время',
  'control.scheduleBuilder.venueColumn': 'Площадка',
  'control.scheduleBuilder.seriesHeading': 'Серия из {span} матчей — сыграно {played}',
  'control.scheduleBuilder.seriesGame': 'Матч {number} из {span}',
  'control.scheduleBuilder.contingencyCertain': 'Будет сыгран',
  'control.scheduleBuilder.contingencyContingent': 'Играется только если серия ещё не решена',
  'control.scheduleBuilder.contingencyNotRequired': 'Больше не нужен — серия уже решена',
  'control.scheduleBuilder.releasedSlot': 'Занимал {slot}. Этот слот свободен.',
  'control.scheduleBuilder.releasedSlotUnknown': 'Не занимал слот на момент решения серии.',
  'control.scheduleBuilder.pendingReleasesHeading': 'Слоты, которые освободит это решение серии',
  'control.scheduleBuilder.pendingRelease': 'Матч {number} — {slot}',
  'control.orgIdentity.heading': 'Идентичность организации',
  'control.orgIdentity.loading': 'Загрузка…',
  'control.orgIdentity.loadFailed': 'Не удалось загрузить настройки организации.',
  'control.orgIdentity.name': 'Название',
  'control.orgIdentity.save': 'Сохранить',
  'control.orgIdentity.saved': 'Организация обновлена.',
  'control.orgIdentity.saveFailed': 'Запрос отклонён.',
  'control.orgIdentity.uploadEmblem': 'Загрузить эмблему',
  'control.orgIdentity.emblemAlt': 'Эмблема организации',
  'control.orgIdentity.emblemPlaceholderAlt': 'Эмблема не загружена',
  'control.orgIdentity.emblemUploaded': 'Эмблема загружена.',
  'control.statisticsRebuild.heading': 'Пересчёт статистики',
  'control.statisticsRebuild.description':
    'Пересчитывает все сохранённые итоги статистики на основе зафиксированных событий. Матчи, сыгранные до выбора состава, дают только показатели на уровне команды, без показателей на уровне игрока, поскольку они никогда не фиксировались.',
  'control.statisticsRebuild.tournamentLabel': 'Турнир (необязательно)',
  'control.statisticsRebuild.tournamentPlaceholder': 'Вся организация',
  'control.statisticsRebuild.trigger': 'Пересчитать статистику',
  'control.statisticsRebuild.confirmPrompt':
    'Это пересчитает все сохранённые показатели в выбранной области. Продолжить?',
  'control.statisticsRebuild.confirm': 'Подтвердить пересчёт',
  'control.statisticsRebuild.cancel': 'Отмена',
  'control.statisticsRebuild.result':
    '{matches, plural, one {Обработан # матч} few {Обработано # матча} many {Обработано # матчей} other {Обработано # матча}}.',
  'control.statisticsRebuild.failed': 'В пересчёте отказано.',
  'control.storageUsage.heading': 'Использование хранилища',
  'control.storageUsage.description':
    'Общий объем хранилища, используемый загруженными медиафайлами и ресурсами в этой организации.',
  'control.storageUsage.loading': 'Загрузка сведений об использовании хранилища…',
  'control.storageUsage.loadFailed': 'Не удалось загрузить данные об использовании хранилища.',
  'control.storageUsage.summary':
    '{formattedBytes} в {objectCount, plural, one {# файле} few {# файлах} many {# файлах} other {# файлах}}',
  'control.storageUsage.empty': '0 MB в 0 файлах',
  'control.storageUsage.unreferencedHeading': 'Неиспользуемые файлы',
  'control.storageUsage.deleteObject': 'Удалить',
  'control.imageCropModal.title': 'Настроить изображение',
  'control.imageCropModal.zoom': 'Масштаб',
  'control.imageCropModal.rotation': 'Поворот',
  'control.imageCropModal.cancel': 'Отмена',
  'control.imageCropModal.confirm': 'Использовать изображение',
  'control.imageCropModal.close': 'Закрыть',
  'control.imageCropModal.processing': 'Обработка…',
  'control.imageCropModal.failed': 'Не удалось обработать это изображение.',
  'control.toast.dismiss': 'Закрыть уведомление',
  'control.toast.notifications': 'Уведомления',
  'control.toast.details': 'Технические подробности',
  'control.toast.severity.success': 'Успешно',
  'control.toast.severity.error': 'Ошибка',
  'control.toast.severity.info': 'Информация',
  'control.apiError.generic': 'Не удалось выполнить запрос. Повторите попытку.',
  'control.apiError.badRequest': 'Проверьте отправленные данные и повторите попытку.',
  'control.apiError.unauthorized': 'Сеанс истёк. Войдите снова.',
  'control.apiError.forbidden': 'У вас нет разрешения на это действие.',
  'control.apiError.notFound': 'Запрошенный объект не найден.',
  'control.apiError.conflict':
    'Изменение конфликтует с текущими данными турнира. Обновите страницу и повторите попытку.',
  'control.apiError.unprocessableEntity': 'Не удалось обработать отправленные данные.',
  'control.apiError.serviceUnavailable':
    'Сервис временно недоступен. Повторите попытку немного позже.',
  'control.apiError.internalServer': 'Произошла непредвиденная ошибка сервера. Повторите попытку.',
  'control.wizard.step.rules': 'Правила событий',
  'control.wizard.problem.chooseAction': 'Выберите действие для правила события',
  'control.wizard.problem.completeRule': 'Заполните обязательные значения по принятой схеме',
  'control.wizard.rule.enable': 'Добавить правило для каждого записанного события',
  'control.wizard.rule.hookHelp':
    'Выполняется синхронно в event.recorded с принятым сервером словарём.',
  'control.wizard.rule.condition': 'Условие',
  'control.wizard.rule.action': 'Действие',
  'control.wizard.rule.chooseAction': 'Выберите действие',
  'control.wizard.rule.conditionAlways': 'Без условия — выполнять для каждого события',
  'control.wizard.rule.conditionlessExplanation':
    'Без условия правило срабатывает для каждого записанного события.',
  'control.wizard.rule.options': 'Параметры (JSON)',
  'control.wizard.rule.addAnother': 'Добавить ещё одно правило',
  'control.wizard.rule.remove': 'Удалить',

  'control.descriptor.wizardTitle': 'Создать дисциплину',
  'control.descriptor.wizardSteps': 'Шаги',
  'control.descriptor.stepName': 'Название',
  'control.descriptor.stepAuthorship': 'Авторство',
  'control.descriptor.stepParticipants': 'Участники',
  'control.descriptor.stepStatistics': 'Статистика и события',
  'control.descriptor.stepFormats': 'Форматы',
  'control.descriptor.stepWinCondition': 'Условие победы',
  'control.descriptor.fieldAlias': 'Псевдоним',
  'control.descriptor.decisionAlias':
    'Устойчивый идентификатор, под которым устанавливается эта дисциплина. Слова в нижнем регистре через дефис; после установки изменить нельзя.',
  'control.descriptor.fieldVersion': 'Версия',
  'control.descriptor.fieldName': 'Название',
  'control.descriptor.fieldDescription': 'Описание',
  'control.descriptor.translationHelp':
    'Английский обязателен. Любой другой язык, оставленный пустым, для такого читателя переключается на английский.',
  'control.descriptor.problemAliasFormat':
    'Псевдоним должен состоять из слов в нижнем регистре через дефис.',
  'control.descriptor.problemVersion': 'Версия обязательна.',
  'control.descriptor.problemNameEnglish': 'Требуется название на английском языке.',
  'control.descriptor.fieldAuthor': 'Автор',
  'control.descriptor.decisionAuthor':
    'Указывается как автор этой дисциплины. Никогда не подставляется по умолчанию из личности инсталляции — без этого публикация отклоняется.',
  'control.descriptor.fieldLicence': 'Лицензия',
  'control.descriptor.decisionLicence':
    'Лицензия, под которой публикуется эта дисциплина, например AGPL-3.0-only.',
  'control.descriptor.fieldSourceUrl': 'URL источника (необязательно)',
  'control.descriptor.problemAuthor': 'Автор обязателен.',
  'control.descriptor.problemLicence': 'Лицензия обязательна.',
  'control.descriptor.fieldParticipantTypes': 'Типы участников',
  'control.descriptor.decisionParticipantTypes':
    'Является ли участник отдельным человеком или целой командой. Это определяет, из чего состоит состав.',
  'control.descriptor.fieldMinPlayers': 'Минимум игроков',
  'control.descriptor.fieldMaxPlayers': 'Максимум игроков',
  'control.descriptor.decisionRosterConstraints':
    'Сколько игроков может быть в составе — проверяется при регистрации и на чек-ине.',
  'control.descriptor.fieldAllowMidTournamentChanges':
    'Разрешить изменения состава во время турнира',
  'control.descriptor.problemParticipantTypes': 'Выберите хотя бы один тип участника.',
  'control.descriptor.problemRosterConstraints':
    'Максимум игроков должен быть не меньше минимума, и оба значения — не меньше 1.',
  'control.descriptor.segmentTypesHeading': 'Типы сегментов',
  'control.descriptor.decisionSegmentTypes':
    'Фазы, на которые делится матч (таймы, партии, раунды). Необязательно — дисциплина, решаемая в одном непрерывном матче, не объявляет ни одного.',
  'control.descriptor.fieldSegmentTypeName': 'Название сегмента',
  'control.descriptor.fieldSegmentTypeLabel': 'Метка сегмента',
  'control.descriptor.fieldSegmentTimed': 'С хронометражем',
  'control.descriptor.add': 'Добавить',
  'control.descriptor.remove': 'Удалить',
  'control.descriptor.statisticsHeading': 'Статистика',
  'control.descriptor.decisionStatistics':
    'Что подсчитывается и как накапливается на протяжении соревнования — это словарь, из которого строятся турнирные таблицы. Дисциплина, ничего не объявляющая, объявляет дисциплину без таблицы.',
  'control.descriptor.fieldStatisticCode': 'Код статистики',
  'control.descriptor.fieldStatisticLabel': 'Метка статистики',
  'control.descriptor.fieldAggregation': 'Режим агрегации',
  'control.descriptor.eventsHeading': 'Определения событий',
  'control.descriptor.decisionEvents':
    'Что может быть зафиксировано во время матча. Событие может начислять одну из указанных выше статистик при каждом наступлении.',
  'control.descriptor.fieldEventCode': 'Код события',
  'control.descriptor.fieldEventLabel': 'Метка события',
  'control.descriptor.fieldEventCategory': 'Категория события',
  'control.descriptor.fieldEventActorRequirement': 'Требование к субъекту',
  'control.descriptor.fieldEventAwardsStatistic': 'Начисляет статистику',
  'control.descriptor.fieldEventAwardsDelta': 'Начисляемое количество',
  'control.descriptor.eventAwardsNone': 'Ничего не начисляет',
  'control.descriptor.problemNoStatistics': 'Объявите хотя бы одну статистику.',
  'control.descriptor.problemEventUndeclaredStatistic':
    'Событие начисляет статистику, которая больше не объявлена. Удалите его или снова объявите статистику.',
  'control.descriptor.fieldAvailableFormats': 'Доступные форматы',
  'control.descriptor.decisionFormats':
    'В каких форматах турнира организатор может провести эту дисциплину. Можно объявлять только форматы, поддерживаемые платформой.',
  'control.descriptor.problemNoFormats': 'Выберите хотя бы один формат.',
  'control.descriptor.scoringInputsHeading': 'Входные данные для подсчёта очков',
  'control.descriptor.decisionScoringInputs':
    'Необязательные именованные значения, которые отчёт о матче может нести помимо зафиксированных событий — производные от событий или введённые оператором напрямую.',
  'control.descriptor.fieldScoringInputCode': 'Код входного значения',
  'control.descriptor.fieldScoringInputLabel': 'Метка входного значения',
  'control.descriptor.fieldScoringInputSource': 'Источник входного значения',
  'control.descriptor.fieldWinConditionMode': 'Форма условия победы',
  'control.descriptor.decisionWinConditionMode':
    'Простая: матч решает единственный счёт (футбол). Сегментированная: сегменты (партии, геймы) закрываются первыми, и матч решается по выигранным сегментам (теннис).',
  'control.descriptor.winConditionModeSimple': 'Простая — матч решает единственный счёт',
  'control.descriptor.winConditionModeSegmented':
    'Сегментированная — сначала закрываются сегменты, затем матч',
  'control.descriptor.fieldSegmentMargin': 'Требуемый отрыв для закрытия сегмента (необязательно)',
  'control.descriptor.fieldSegmentName': 'Закрываемый сегмент',
  'control.descriptor.fieldSegmentTarget': 'Единиц требуется для закрытия сегмента',
  'control.descriptor.fieldTiebreakAt': 'Тай-брейк начинается при (необязательно)',
  'control.descriptor.fieldTiebreakTarget': 'Цель тай-брейка (необязательно)',
  'control.descriptor.fieldTiebreakMargin': 'Отрыв в тай-брейке (необязательно)',
  'control.descriptor.fieldWinMatchUnitSimple': 'Статистика, решающая матч',
  'control.descriptor.fieldWinMatchUnitSegmented': 'Сегмент, засчитываемый для решения матча',
  'control.descriptor.decisionWinMatchUnit':
    'Что решает матч — объявленный код статистики в простом режиме или название закрытого сегмента в сегментированном режиме.',
  'control.descriptor.fieldWinMatchTarget': 'Цель, необходимая для победы (необязательно)',
  'control.descriptor.decisionWinMatchTarget':
    'Оставьте пустым для матча, который закрывается только по окончании основного времени с явным лидером (равный счёт остаётся ничьей). Укажите число для «первый до N».',
  'control.descriptor.problemWinConditionUnit': 'Выберите, что решает матч.',
  'control.descriptor.problemSegmentName': 'Выберите, какой сегмент закрывается.',
  'control.descriptor.problemSegmentTarget':
    'Количество единиц для закрытия сегмента должно быть не меньше 1.',
  'control.descriptor.problemSegmentUndeclared':
    'Закрываемый сегмент должен быть одним из объявленных на шаге участников.',
  'control.descriptor.back': 'Назад',
  'control.descriptor.continue': 'Далее',
  'control.descriptor.authorAndInstall': 'Создать и установить',

  'control.profile.wizardTitle': 'Создать профиль турнира',
  'control.profile.wizardSteps': 'Шаги',
  'control.profile.stepName': 'Название',
  'control.profile.stepAuthorship': 'Авторство',
  'control.profile.stepStages': 'Этапы',
  'control.profile.stepPoints': 'Очки',
  'control.profile.fieldAlias': 'Псевдоним',
  'control.profile.decisionAlias':
    'Устойчивый идентификатор, под которым устанавливается этот профиль. Слова в нижнем регистре через дефис; после установки изменить нельзя.',
  'control.profile.fieldVersion': 'Версия',
  'control.profile.fieldName': 'Название',
  'control.profile.fieldDescription': 'Описание',
  'control.profile.problemAliasFormat':
    'Псевдоним должен состоять из слов в нижнем регистре через дефис.',
  'control.profile.problemVersion': 'Версия обязательна.',
  'control.profile.problemNameEnglish': 'Требуется название на английском языке.',
  'control.profile.fieldAuthor': 'Автор',
  'control.profile.decisionAuthor':
    'Указывается как автор этого профиля. Никогда не подставляется по умолчанию из личности инсталляции — без этого публикация отклоняется.',
  'control.profile.fieldLicence': 'Лицензия',
  'control.profile.fieldSourceUrl': 'URL источника (необязательно)',
  'control.profile.problemAuthor': 'Автор обязателен.',
  'control.profile.problemLicence': 'Лицензия обязательна.',
  'control.profile.fieldDiscipline': 'Проверять форматы этапов по',
  'control.profile.decisionDiscipline':
    'Установленная дисциплина, по которой проверяется формат каждого этапа. Сам профиль никогда не называет дисциплину — его можно использовать с любой дисциплиной, чьи форматы совпадают.',
  'control.profile.problemDiscipline':
    'Выберите дисциплину, по которой проверяются форматы этапов.',
  'control.profile.stagesHeading': 'Этапы',
  'control.profile.decisionStages':
    'Этапы соревнования по порядку — например, групповой этап, ведущий к плей-офф. Формат каждого этапа должен быть одним из объявленных выбранной дисциплиной.',
  'control.profile.fieldStageName': 'Название этапа',
  'control.profile.fieldStageFormat': 'Формат этапа',
  'control.profile.problemNoStages': 'Объявите хотя бы один этап.',
  'control.profile.problemStageFormat':
    'Формат каждого этапа должен быть одним из объявленных выбранной дисциплиной.',
  'control.profile.fieldPointsWin': 'Очки за победу',
  'control.profile.fieldPointsDraw': 'Очки за ничью',
  'control.profile.fieldPointsLoss': 'Очки за поражение',
  'control.profile.problemNegativePoints': 'Очки не могут быть отрицательными.',
  'control.profile.add': 'Добавить',
  'control.profile.remove': 'Удалить',
  'control.profile.back': 'Назад',
  'control.profile.continue': 'Далее',
  'control.profile.authorAndInstall': 'Создать и установить',

  'control.matchesView.title': 'Матчи',
  'control.matchesView.loadFailed': 'Не удалось загрузить матчи.',
  'control.matchesView.filterAll': 'Все',
  'control.matchesView.filterLive': 'В эфире',
  'control.matchesView.filterUpcoming': 'Предстоящие',
  'control.matchesView.filterFinal': 'Завершённые',
  'control.matchesView.empty': 'В этой области пока нет матчей.',
  'control.matchesView.clockAriaLabel': 'Прошло времени: {time}',
  'control.matchesView.venueAriaLabel': 'Площадка: {venue}',
  'control.matchesView.latestEventAriaLabel': 'Последнее событие: {event}',
  'control.matchesView.zoneGroupAriaLabel': 'Зона/группа: {scope}',
  'control.matchesView.positionInGroup': '{group} — позиция №{position}',
  'control.matchesView.position': 'Позиция №{position}',
  'control.matchesView.decidedBy': 'Решено по: {factor}',
  'control.matchesView.decidedByAriaLabel':
    'Решающий фактор между позициями с равными показателями.',
  'control.matchesView.fullTraceHeading': 'Полная трассировка компаратора турнирной таблицы',
  'control.matchesView.resultState.live': 'В ЭФИРЕ',
  'control.matchesView.resultState.upcoming': 'ЗАПЛАНИРОВАНО',
  'control.matchesView.resultState.final': 'ИТОГ',
  'control.matchesView.resultState.disputed': 'ОСПАРИВАЕТСЯ',
  'control.matchesView.resultState.winner': 'ПОБЕДА',
  'control.matchesView.resultState.loser': 'ПОРАЖЕНИЕ',
  'control.matchesView.resultState.tbd': 'УТОЧНЯЕТСЯ',
  'control.matchesView.resultState.cancelled': 'ОТМЕНЕНО',
  'control.matchesView.series.ariaLabel': 'Серия до {bestOf} побед: {home} — {away}',
  'control.matchesView.series.pending': 'Серия не решена при {home}–{away}',
  'control.matchesView.series.decided': '{winner} выиграл серию',
  'control.matchesView.series.aggregate': 'По сумме {home}–{away}',
  'control.matchesView.seeAll': 'Смотреть все матчи',

  'control.dashboard.open': 'Открыть',
  'control.dashboard.export': 'Экспорт',
  'control.dashboard.resumeEditing': 'Продолжить редактирование',

  // 0223 — standings panel chrome, metric availability, module updates.
  'control.standings.panelDecidedBy': 'Определил это место',
  'control.standings.tiebreakerSequenceTitle': 'Последовательность тай-брейков',
  'control.standings.rankColumn': 'Место',
  'control.metric.unavailable': 'Нет данных',
  'control.metric.demonstration': 'Демонстрационное значение',
  'control.platform.modules.updateEyebrow': 'Обновление модуля',
  'control.platform.modules.updateHeadline': '{alias} {currentVersion} → {latestVersion}',
  'control.platform.modules.updateKind': 'Тип обновления: {upgrade}',
  'control.platform.modules.updateCalloutTitle': 'Установить эту версию',
  'control.platform.modules.updateCalloutDescription':
    'Введите псевдоним ниже и установите его. Модули устанавливаются через приложение начальной загрузки; из этого списка ничего не применяется.',
  'control.auditTrail.correctionTitle': 'Что изменила эта запись',
  'control.descriptor.documentRegion': 'Составленный документ модуля',
  'control.descriptor.documentCopy': 'Скопировать документ модуля',
  'control.descriptor.documentCopied': 'Скопировано',
  'control.descriptor.documentCopyFailed':
    'Не удалось скопировать — выделите текст, чтобы скопировать его',
  'control.liveConsole.subtitle': 'Консоль текущих операций для турниров и матчей в игре.',
  'control.liveConsole.loading': 'Загрузка текущих операций…',
  'control.liveConsole.noActiveTournaments': 'В этой организации нет активных турниров.',
  'control.liveConsole.createTournament': 'Создать турнир',
  'control.preferences.tokensLoading': 'Загрузка…',
  'preferences.title': 'Личные настройки',
  'preferences.patTitle': 'Персональные токены доступа',
  'preferences.patDescription':
    'Создайте токен для прямого доступа к API. Токен будет показан только один раз.',
  'preferences.createPat': 'Создать токен',
  'preferences.patLabel': 'Название токена',
  'preferences.patExpiresIn': 'Срок действия (дней)',
  'preferences.patCreated': 'Токен создан. Скопируйте его сейчас:',
  'preferences.revokePat': 'Отозвать',
  'preferences.noTokens': 'Нет активных персональных токенов доступа.',
  'preferences.patExpiresAt': 'Действует до: {date}',
  'control.analytics.subtitle': 'Показатели эффективности и объёма операций организации.',
  'control.analytics.loading': 'Загрузка аналитики…',
  'control.analytics.totalTournaments': 'Всего турниров',
  'control.analytics.tournamentsBreakdown':
    '{live} в игре · {upcoming} предстоящих · {finished} завершённых',
  'control.analytics.finishedTournaments': 'Завершённые турниры',
  'control.analytics.finishedDetail': 'Заархивированные и завершённые результаты',
  'control.analytics.storageUsed': 'Использовано хранилища',
  'control.analytics.mediaFilesCount': '{count} медиафайлов',
  'control.analytics.noData': 'Нет данных',
  'control.analytics.tournamentOverview': 'Обзор турниров',
  'control.analytics.noTournaments': 'Турниры не найдены.',
  'control.analytics.column.tournament': 'Турнир',
  'control.analytics.column.status': 'Статус',
  'control.analytics.column.progress': 'Прогресс матчей',
  'control.analytics.column.liveMatches': 'Матчи в эфире',
  'control.analytics.matchProgress': '{resolved} / {total} матчей',
  'control.analytics.status.draft': 'Черновик',
  'control.analytics.status.upcoming': 'Предстоящий',
  'control.analytics.status.live': 'В эфире',
  'control.analytics.status.finished': 'Завершён',
  'control.analytics.status.archived': 'Архивный',
  'control.analytics.status.unknown': 'Неизвестно',
  'control.roster.rolePlayer': 'Игрок',
  'control.roster.roleSubstitute': 'Запасной',
  'control.roster.roleCoach': 'Тренер',
  'control.roster.roleStaff': 'Персонал',
  'control.roster.noMembersYet': 'В этой команде пока нет участников.',
  'control.roster.roleFieldLabel': 'Роль {name}',
  'control.auditLogPanel.eventCount':
    '{count, plural, one {# событие} few {# события} many {# событий} other {# события}}',
  'control.auditLogPanel.latency': 'Задержка {latencyMs}мс',
  'control.tournament.completion': 'Завершение турнира',
  'control.tournament.completionStage': 'Этап {stageNumber}',

  'control.disciplineSummary.segmentsHeading': 'Сегменты',
  'control.disciplineSummary.noSegments': 'Эта дисциплина не определяет сегменты.',
  'control.disciplineSummary.segmentTimed':
    '{name}: идёт отсчёт времени. Длительность по умолчанию: {duration}.',
  'control.disciplineSummary.segmentUntimed': '{name}: без отсчёта времени.',
  'control.disciplineSummary.rulesHeading': 'Правила',
  'control.disciplineSummary.noRules': 'Эта дисциплина не определяет настраиваемые правила.',
  'control.disciplineSummary.ruleCurrentValue': 'Текущее значение: {value}',
  'control.disciplineSummary.permissionInherited':
    'Использует значение дисциплины по умолчанию, если турнир его не заменяет.',
  'control.disciplineSummary.permissionReplaced':
    'Может быть заменено значением, специфичным для турнира.',
  'control.disciplineSummary.permissionMerged':
    'Может быть дополнено собственными значениями турнира, в дополнение к значениям дисциплины.',
  'control.disciplineSummary.permissionForbidden': 'Нельзя изменить для отдельного турнира.',
  'control.disciplineSummary.eventsHeading': 'События',
  'control.disciplineSummary.noEvents': 'Эта дисциплина не определяет события.',
  'control.disciplineSummary.eventAffectsResult': 'Меняет результат',
  'control.disciplineSummary.eventActorSide': 'Относится к команде.',
  'control.disciplineSummary.eventActorPerson': 'Относится к игроку.',
  'control.disciplineSummary.eventActorPersonOrStaff': 'Относится к игроку или члену персонала.',
  'control.disciplineSummary.eventActorNone': 'Не относится к конкретному участнику команды.',
  'control.disciplineSummary.rawJsonToggleShow': 'Показать необработанный JSON',
  'control.disciplineSummary.rawJsonToggleHide': 'Скрыть необработанный JSON',
  'control.tournamentSummary.factsHeading': '{name}',
  'control.tournamentSummary.stagesHeading': 'Этапы',
  'control.tournamentSummary.stageLine': '{name}: {format}',
  'control.tournamentSummary.publicRegistrationOpen': 'Публичная регистрация открыта.',
  'control.tournamentSummary.publicRegistrationClosed':
    'Публичная регистрация закрыта; участников добавляет организатор.',
  'control.tournamentSummary.checkInRequired': 'Регистрация на месте обязательна.',
  'control.tournamentSummary.checkInNotRequired': 'Регистрация на месте не обязательна.',
  'control.tournamentSummary.checkInClosesAt': 'Регистрация на месте закрывается в {closesAt}.',
  'control.tournamentSummary.region': 'Регион: {region}',
  'control.tournamentSummary.capacity': 'Вместимость: {capacity} участников',
  'control.disciplineDocument.loading': 'Загрузка дисциплины…',
  'control.disciplineDocument.loadFailed': 'Не удалось загрузить эту дисциплину.',
  'control.platform.viewDiscipline': 'Просмотр',
  'control.rulesetField.listAdd': 'Добавить',
  'control.rulesetField.inheritedHeading': 'Уже включает:',
  'control.rulesetField.unrecognized':
    'Не регулируется известной политикой правил — редактируется как необработанный JSON.',
  'control.rulesetField.unknownType':
    'Тип значения этого поля неизвестен — редактируется как необработанный JSON.',

  // invitation.* (AcceptInvitationForm.tsx / accept.astro)
  'invitation.missingToken': 'Токен приглашения не найден в ссылке.',
  'invitation.passwordTooShort': 'Пароль должен содержать не менее 8 символов.',
  'invitation.passwordMismatch': 'Пароли не совпадают.',
  'invitation.acceptFailed': 'Не удалось принять приглашение ({status}).',
  'invitation.unexpectedError': 'Непредвиденная ошибка при принятии приглашения.',
  'invitation.title': 'Принять приглашение',
  'invitation.subtitle': 'Настройте свою учётную запись администратора CopaLibre',
  'invitation.successHeading': 'Учётная запись настроена!',
  'invitation.successBody': 'Перенаправление в панель управления…',
  'invitation.nameLabel': 'Полное имя (необязательно)',
  'invitation.namePlaceholder': 'Напр. Ana Pérez',
  'invitation.passwordLabel': 'Пароль (минимум 8 символов)',
  'invitation.confirmPasswordLabel': 'Подтвердите пароль',
  'invitation.submitLoading': 'Настройка учётной записи…',
  'invitation.submit': 'Принять и начать',

  'control.disciplineSummary.segmentsFieldValue':
    '{count, plural, one {# основной период} few {# основных периода} many {# основных периодов} other {# основного периода}} ({overtime, select, true {с овертаймом} other {без овертайма}})',
  'control.booleanYes': 'Да',
  'control.booleanNo': 'Нет',
  'control.filePicker.prompt': 'Выберите файл или перетащите его сюда',
  'control.filePicker.promptDragging': 'Отпустите файл здесь',
  'control.filePicker.acceptedFormats': 'Допустимые форматы: {formats}',
  'control.filePicker.maxSize': 'Максимальный размер: {size}',
  'control.filePicker.clear': 'Очистить',
  'control.filePicker.filesSelected':
    '{count, plural, one {# файл выбран} few {# файла выбрано} many {# файлов выбрано} other {# файла выбрано}}',
};
