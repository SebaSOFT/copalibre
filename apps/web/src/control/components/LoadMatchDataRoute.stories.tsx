...
function Screen({ mode }: { readonly mode: 'loaded' | 'empty' | 'loading' | 'failed' }) {
  ...
  const client = useMemo(() => {
    ...
    function read<T>(value: T): Promise<T> {
      ...
      if (mode === 'failed')
        return Promise.reject(new Error(intl.formatMessage(messages.loadMatchDataLoadFailed)));
    ...
    return storyClient<MatchConsoleApiClient>({
      fetchMatchConsole: () =>
        read({
          ...
          events: [],
          rosters: [],
          capabilities: empty ? [] : consoleProjection.capabilities,
        }),
...
const meta = {
  title: 'Admin/Screens/LoadMatchDataRoute',
  component: Screen,
  args: { mode: 'loaded' },
