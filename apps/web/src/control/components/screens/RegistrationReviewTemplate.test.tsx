...
import {
  ...
  type ReviewRegistrationRow,
} from '../screens/RegistrationReviewTemplate.js';
...
function renderPage(overrides: Partial<Parameters<typeof RegistrationReviewTemplate>[0]> = {}) {
...
describe('RegistrationReviewTemplate — nationality and profile', () => {
  ...
  it('opens EditTeamMembersDialog, allows changing member role to coach, and saves', async () => {
    const onEditTeamMembers = jest.fn<
      NonNullable<Parameters<typeof RegistrationReviewTemplate>[0]['onEditTeamMembers']>
    >(() => Promise.resolve());
    renderPage({
      onEditTeamMembers,
      rows: [
