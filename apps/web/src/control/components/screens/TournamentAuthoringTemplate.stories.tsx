...
import { storyClient, pending, ORG, discipline } from '../screen-story-fixtures.js';
...
function Screen({ mode }: { readonly mode: 'loaded' | 'empty' | 'loading' | 'failed' }) {
  ...
  const client = useMemo(() => {
    ...
    function read<T>(value: T): Promise<T> {
      ...
      if (mode === 'failed')
        return Promise.reject(new Error(intl.formatMessage(messages.authoringLoadFailed)));
      return Promise.resolve(value);
    }
    return storyClient<ControlApiClient>({
      listDisciplines: () => read(empty ? [] : [discipline]),
      fetchCustomScriptVocabulary: undefined,
      listCompatibleProfiles: undefined,
    });
  }, [mode, intl]);
  return <TournamentAuthoringTemplate organizationAlias={ORG} client={client} />;
}
const meta = {
  title: 'Admin/Screens/TournamentAuthoringTemplate',
  component: Screen,
  args: { mode: 'loaded' },
