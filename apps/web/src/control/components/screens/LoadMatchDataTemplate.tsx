 ...
 */
export function LoadMatchDataTemplate({
  candidatesByEntrant,
  matchId,
  onSubmit,
  projection,
  ...
  readonly candidatesByEntrant: ReadonlyMap<string, readonly RosterCandidate[]>;
  readonly matchId: string;
  readonly onSubmit: (request: BulkLoadMatchDataRequest) => Promise<boolean>;
  ...
  const [selections, setSelections] = useState<Record<string, Record<string, MemberSelection>>>(
    () =>
      Object.fromEntries(
        projection.entrants.map(({ entrantId }) => [
          entrantId,
          Object.fromEntries(
            (candidatesByEntrant.get(entrantId) ?? []).map((candidate) => [
              candidate.personId,
