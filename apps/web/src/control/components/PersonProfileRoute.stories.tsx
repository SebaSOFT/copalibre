...
function Screen({ mode }: { readonly mode: 'loaded' | 'empty' | 'loading' | 'failed' }) {
  ...
  const client = useMemo(() => {
    ...
    function read<T>(value: T): Promise<T> {
      ...
      if (mode === 'failed')
        return Promise.reject(new Error(intl.formatMessage(messages.personProfileLoadFailed)));
    ...
    return storyClient<ControlApiClient>({
      getPerson: () =>
        read({
          personId: ids.person,
          displayName: 'V. Kael',
          ...(empty ? {} : { nationality: 'AR', naturalKey: { kind: 'member', value: 'MS-014' } }),
        }),
    });
  }, [mode, intl]);
  return <PersonProfileRoute organizationAlias={ORG} personId={ids.person} client={client} />;
}
const meta = {
  title: 'Admin/Screens/PersonProfileRoute',
  component: Screen,
  args: { mode: 'loaded' },
