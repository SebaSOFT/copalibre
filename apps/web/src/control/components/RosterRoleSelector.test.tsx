...
describe('RosterRoleSelector', () => {
  ...
  it('renders all members and their current role badges', () => {
    ...
    render(withIntl(<RosterRoleSelector members={sampleMembers} onChange={onChange} />));
  ...
  it('renders empty state when members array is empty', () => {
    const onChange = jest.fn();
    render(withIntl(<RosterRoleSelector members={[]} onChange={onChange} />));

    expect(screen.getByText(/No members on this team yet/)).toBeDefined();
  });
