...
- `/help/**` Starlight documentation routes (Markdown/MDX, navigation, table of contents, search,
  i18n).
- `/help/api-reference/` interactive OpenAPI reference via `@scalar/astro` with
  `renderMode="client"` (required per Scalar's own guidance — static mode can leave the reference
  blank after Starlight/view-transition navigation).
