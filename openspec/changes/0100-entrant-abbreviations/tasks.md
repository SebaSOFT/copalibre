...
- [x] 1.1 No new value object. `Entrant`'s abbreviation is validated with the existing
...
- [x] 1.2 `packages/domain/src/aggregates/participant.ts`: `Entrant` gains `readonly abbreviation?:
      string`.
- [x] 1.3 `packages/domain/src/aggregates/short-labels.ts`: `STOP_WORDS` — English, Spanish, Portuguese,
...
- [x] 1.4 Unit tests: `deriveEntrantAbbreviation` cases for each priority branch, truncation, and the
...
- [x] 2.1 Migration: `ALTER TABLE entrants ADD COLUMN abbreviation text NULL`; partial unique index on
      `(tournament_id, abbreviation) WHERE abbreviation IS NOT NULL`.
- [x] 2.2 `packages/persistence/src/schema.ts`: `EntrantsTable` gains `abbreviation: string | null`.
- [x] 2.3 `packages/persistence/src/mapping.ts`: `toEntrant` carries `abbreviation`.
- [x] 2.4 `packages/persistence/src/repositories/enrollment-repository.ts`: `registerEntrant` —
...
- [x] 2.5 New method `setEntrantAbbreviation(uow, { entrantId, abbreviation, organizationId, actor,
...
- [x] 2.6 New read `listEntrantsNeedingAbbreviation(tournamentId)`: entrants in the tournament with
      `abbreviation IS NULL` — the "needs an abbreviation" list an officer works from.
- [x] 2.7 Integration tests: registering two entrants whose derived candidates collide leaves the
...
- [x] 3.1 `apps/api/src/dto/` and `packages/domain/src/import-export/csv-import.ts` — entrant
...
- [x] 3.2 New route: `PATCH .../entrants/:entrantId/abbreviation` (mirrors the existing entrant/
      ...
      act on.
- [x] 3.3 New route: `GET .../tournaments/:tournamentAlias/entrants/needing-abbreviation`, calling
      `listEntrantsNeedingAbbreviation`.
- [x] 3.4 Regenerate `packages/contracts/openapi/v1.json` and `src/generated/v1.ts`.
- [x] 3.5 Integration tests for both new routes, plus CSV import of valid, malformed, and
...
- [x] 4.1 `apps/api/src/controllers/public-projections.controller.ts`: the `names`/`standingsNames`
...
- [x] 4.2 Integration test: a bracket/standings/overview response for an entrant with its own resolved
      abbreviation shows that value, not the team's, when they differ.

## 5. Web — a shared, size-aware name component

- [x] 5.1 New shared component (e.g. `EntrantName`) taking the full name and the resolved abbreviation:
...
- [x] 5.2 `apps/web/src/control/components/StandingsPage.tsx` (name/abbreviation cell rendering),
...
- [x] 5.3 Component/unit tests: mock `ResizeObserver` to verify full name at generous width and the
...
- [x] 6.1 Confirm new `*.test.ts`/`*.integration.test.ts` files are picked up by existing Jest glob
      configuration; no explicit registration expected.
- [x] 6.2 No `.github/workflows/ci.yml` change expected — new test files under existing steps' glob
      only; confirm with `yarn test:verify-discovery` before merging.
- [x] 6.3 e2e: one Playwright case confirming an entrant's abbreviation renders with a working tooltip
