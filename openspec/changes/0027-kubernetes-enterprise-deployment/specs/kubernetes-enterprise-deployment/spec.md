...
that installing the chart with default values produces identical behavior to `0026-k3s-helm-deployment`
alone.
...
- **THEN** the restored installation passes the same integrity checks defined for the Compose-level backup/restore requirement in `0021-deployment-docker-compose-cli`
...
- **THEN** the deployed image digest matches the digest validated by `0026-k3s-helm-deployment`'s rolling-update test for the same release
