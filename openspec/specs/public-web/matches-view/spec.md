...
### Requirement: Public matches view cards receive explicit locale prop
Every `MatchCard` component mounted in the public matches view SHALL receive an explicit `locale` prop
matching the page's resolved language, ensuring all child timestamp formatters hydrate identically
between server and client.

#### Scenario: MatchCard renders with resolved locale
- **WHEN** a spectator views the public matches view in any supported language
- **THEN** every `MatchCard` on the page is passed the page's active locale
- **AND** no React hydration mismatch error is generated by child `ResponsiveTimestamp` components

### Requirement: Public matches view renders structured empty state
When active filter selections produce zero matching fixtures, the public matches view SHALL render an
owned empty-state component clearly communicating the absence of matches for the selected criteria.

#### Scenario: Filter narrows matches to zero
